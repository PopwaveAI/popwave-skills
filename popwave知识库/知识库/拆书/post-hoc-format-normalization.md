---
id: post-hoc-format-normalization
lib: 知识库
cat: 拆书
version: 1.0.0
tags: [拆书]
...
> **为什么需要**: 不同子 agent 使用不同 heading 格式（`# 第X章·标题 — 设计包 (v4)` vs `# 设计包 — chXXX「标题」`），3层+1区结构小节命名也不同（`beat链层` vs `骨架层` vs `情节层` 等变体）。
...
将以下脚本放在 `_temp/` 下运行。它扫描 `写作资产/设计包v4/` 并修复两种最常见的漂移：
...
SECTION_MAP = {
    # v4 标准命名（3层+1区）
    'beat链层': 'beat链层', 'L1beat链层': 'beat链层',
    '爽点层': '爽点层', 'L2爽点层': '爽点层',
    '角色层': '角色层', 'L3角色层': '角色层',
    '设定/物品提取区': '设定/物品提取区', 'S1设定/物品提取区': '设定/物品提取区',
    # v4 变体 → 设定/物品提取区
    '设定区': '设定/物品提取区', '设定物品区': '设定/物品提取区', '设定提取区': '设定/物品提取区',
    # v3/旧标准历史映射 → beat链层（兼容历史文件）
    '骨架层': 'beat链层', '情节层': 'beat链层', '叙事层': 'beat链层',
    '事件链': 'beat链层', '剧情层': 'beat链层', '事件链层': 'beat链层', 'L1事件链层': 'beat链层',
    # v3 历史映射 → 爽点层（兼容历史文件）
    '爽点设计': '爽点层', '情绪层': '爽点层',
    # v3 历史映射 → 角色层（兼容历史文件）
...
- **v4 beat表列数校验**：v4 beat表应为 **7 列**（`| # | beat | 类型 | scene | POV | 参与角色 | 原文证据 |`），非 v3 的 8 列（v3 多一个「字数估计」列）。归一化时检查beat表列数，若发现 8 列则删除「字数估计」列
