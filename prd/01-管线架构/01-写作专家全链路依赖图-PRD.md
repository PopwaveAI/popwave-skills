...
> **v12.0 迭代**：v2管线转正为默认模式。原 v1 管线封存至 `skills/_deprecated/`。creative v2.0.0（生态图谱替代域DNA+文风DNA立项锁定+金手指行动引擎）；world v2.0.0（金手指数值化以行动引擎为核心+社会结构从生态图谱推导）；character v2.0.0（增加生态位置/生态关系/生态冲突点）；plot v2.0.0（危机交叉设计+事件密度基准+情绪闭环检查）；chapter v2.0.0（场景流设计包·导演指令式≤8K字）；prose v2.0.0（文风DNA硬阻塞+场景流渲染+80%展示20%内省）；qa v2.0.0（密度/多样性/温度量化检查）。expert-writer v7.0.0（移除v1/v2双轨切换）。
> **v11.1 迭代**：creative+reservoir 合并。creative v6.0.0 吸收 reservoir v3.3.0 全部能力。管线顺序去 reservoir，creative 成为立项+素材储备统一入口。
...
-creative  -world    -character -plot     -chapter  -prose   -qa
  v2.0.0    v2.0.0    v2.0.0     v2.0.0    v2.0.0    v2.0.0   v2.0.0
```

**管线设计特点（v2 转正后）：**
- creative：生态图谱替代域DNA + 文风DNA立项锁定 + 金手指行动引擎
- world：金手指数值化以行动引擎为核心 + 社会结构从生态图谱推导
- character：增加生态位置/生态关系/生态冲突点字段
- plot：危机交叉设计 + 事件密度基准 + 情绪闭环检查
- chapter：场景流设计包（导演指令式，≤8K字，非逐事件填表）
- prose：文风DNA硬阻塞 + 场景流渲染 + 80%展示20%内省
- qa：密度/多样性/温度量化检查
...
| Skill | 入 | 出 | S/D | → | 用途 |
|:------|:----|:----|:---:|:---|:-----|
| creative v2.0.0 | 用户 + library + WebSearch | `全书立项PRD.md`（创意决策不写施工图） + `素材储备池/{素材}.md` + `研究档案/`（生态图谱+金手指行动引擎+文风DNA锁定声明） | S/D | world, character, plot | 唯一立项宪法；生态图谱（≥3角色群，替代域DNA）；金手指行动引擎（≥1行动驱动机制：任务/技能/经验/血量/战宠）；文风DNA立项锁定（空=硬阻塞）；闸门：样品确认 |

### 小说设定环节

| Skill | 入 | 出 | S/D | → | 用途 |
|:------|:----|:----|:---:|:---|:-----|
| world v2.0.0 ① L1设定 | PRD + 生态图谱 + library | `小说世界设定/L1-01~07.md` | S | character, plot | 世界蓝图/力量/历史驱力/物种/势力/资源/术语文明底色；社会结构从生态图谱推导 |
| world ② 数值+升级表 | PRD宪法 + L1力量体系 + 金手指行动引擎 | `小说世界设定/数值体系/*.md` `动态升级表.md` | S | character, plot | 战斗数值（金手指数值化以行动引擎为核心）+ 每卷力量膨胀曲线 |
| world ③ 起终点快照 | PRD + L1 + 数值体系 | `小说世界设定/起点快照.md` `终点快照.md` | S | character, plot | 卷1初态 + 卷N终态 |
| world（library获取） | pop-trope-library | `文风库/{书名}.md` | S | prose | 文风DNA档案 |
| character v2.0.0 | PRD + L1 + 数值 + 快照 + 生态图谱 + library | `状态/角色/{主角,配角}-角色卡.md`（含生态位置+生态关系+生态冲突点） | D | plot, chapter | 主角与主要配角设定卡 |

### 剧情设计环节

| Skill | 入 | 出 | S/D | → | 用途 |
|:------|:----|:----|:---:|:---|:-----|
| plot v2.0.0 Step 0 设计意图 | PRD + 角色卡 + 项目总控 | 用户设计意图模型（内存）+ L4全书事件识别 | — | plot Step1 | 主线大类确认+卷节奏+爽点优先级+角色弧线+跨卷大事件识别+危机交叉设计 |
...
| plot Step 6 Canvas验核 | `step-6-canvas-audit.md` + `references/prd-payoff-audit.md` | `幕/vol-XX/L2-{编号}-{单元名}.md`（追加验核报告） | D | chapter | 爽点密度+大爽点间隔+承诺兑现+线间平衡+债务回收+事件密度基准+情绪闭环检查 |

### 正文产出环节

| Skill | 入 | 出 | S/D | → | 用途 |
|:------|:----|:----|:---:|:---|:-----|
| chapter v2.0.0 | L2单元卡 + L3剧情线 + state-log.yaml | `章节设计包/chXXX-设计包.md`（场景流·导演指令式·≤8K字）**+ DQTC决策溯源附录（独立文件）** | D | prose | 场景流设计（3-5场景/章，非逐事件填表）；CH1补充state-log baseline #0角色状态；不写state-log |
| prose v2.0.0 | 设计包 + 文风DNA | `正文/chXXX.md`（场景流渲染）+ `状态/state-log.yaml`（追加event） + 总控更新 | D | qa | 文风DNA硬阻塞（缺失=终止，不降级）；场景流渲染（非逐事件）；80%展示+20%内省；章末追加event到state-log（唯一写入者） |
| qa v2.0.0 | 正文 + 设计包 + L2-{编号} | （不留盘）密度/多样性/温度量化检查 | — | — | 密度量化和多样性量化+温度量化+L1硬门禁→L2三层介入→L3原文对照 |

### 创意环节过程文档的下游消费（★v12.0 更新）

> 研究档案/ 是 creative 的产出，v2.0.0 起以生态图谱替代域DNA，新增金手指行动引擎和文风DNA锁定声明。

| 研究档案文件 | 下游消费者 | 消费字段 | 用途 | 门禁 |
|:---|:---|:---|:---|:---|
| 生态图谱.md | world, character | 角色群清单+5字段+交叉点 | L1设定的社会结构推导+角色生态位置设计 | 必读 |
| 金手指行动引擎.md | world, plot | 5维度行动引擎+剧情驱动+数值风格 | 数值体系设计+剧情危机交叉设计 | 必读 |
| 文风DNA锁定声明.md | prose | 5维度拆解+硬锁定 | 正文渲染的文风锚定（缺失=硬阻塞） | prose必读 |
| 研究档案索引.md | 全部下游 | 文件清单+消费映射 | 导航入口，确定该读哪些文件 | 研究档案/ 存在时必读 |

> 消费协议：下游 skill 进入时先读 研究档案索引.md → 按消费映射表定位所需文件 → 按需读取对应域DNA档案（不全量加载）→ 检查 lastUpdatedAt 时间戳。
...
| `文风库/` | 文风笔触档案（场景卡+通用维度+时间演变） | prose | pop-shared-dna |
| `剧情库/` | **标准剧情线**（六段格式），按内容标签分目录 | plot | 拆书 Phase 2 volume |
| `套路库/` | 抽象叙事模式卡（10字段：模式家族+变体+跨书案例） | creative, plot, chapter, qa | Phase 1 design-pack |
...
| prose | `文风库/{书名}.md` | 风格渲染场景卡（按 scene 匹配） | 正文渲染的文风锚定 |
...
│
├── 研究档案/                            [creative] 生态图谱+金手指行动引擎+文风DNA锁定声明（S→D，下游 world/character/plot/prose 消费）
│   ├── 研究档案索引.md                  [creative] 导航+消费映射
│   ├── 生态图谱.md                      [creative] 角色群清单+5字段+交叉点 → world/character 消费
│   ├── 金手指行动引擎.md                [creative] 5维度行动引擎+剧情驱动+数值风格 → world/plot 消费
│   └── 文风DNA锁定声明.md               [creative] 5维度拆解+硬锁定 → prose 消费
│
├── 小说世界设定/                        [world]
│   ├── L1-01世界蓝图.md
│   ├── L1-02力量体系.md
...
> 本文档基于各 skill 当前 skill.json 版本构建（2026-06-25）。
> 写作专家 skill 清单：expert-writer(v7.0.0)、pop-writer-creative(v2.0.0)、pop-writer-world(v2.0.0)、
> pop-writer-character(v2.0.0)、pop-writer-plot(v2.0.0)、pop-writer-chapter(v2.0.0)、pop-writer-prose(v2.0.0)、pop-writer-qa(v2.0.0)、
> pop-writer-game(v2.0.2)、pop-writer-html(v1.3.2)。
> v1 管线已封存至 `skills/_deprecated/`。
> 状态管理：state-log.yaml（append-only 叙事日志，零脚本，prose 唯一写入者）。
> 未来架构演进方向见 `09-涌现反馈环写作管线-PRD.md`。
