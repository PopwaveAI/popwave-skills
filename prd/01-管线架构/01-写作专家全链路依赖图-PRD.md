...
| Skill | 入 | 出 | S/D | → | 用途 |
|:------|:----|:----|:---:|:---|:-----|
| creative v6.0.0 | 用户 + library + WebSearch | `全书立项PRD.md` + `素材储备池/{素材}.md`（剧情储备卡/设定储备卡） + `研究档案/`（种子展开图+域DNA档案+创意溯源记录+交叉授粉图） | S/D | world, character, plot | 唯一立项宪法（10块）；剧情储备卡（元卖点+冲突引擎+剧情线发展链条8~20节点）；设定储备卡（设定内核+可衍生冲突空间，标注对应L1维度）；闸门：储备池确认 |
...
### 创意环节过程文档的下游消费（★v11.2 新增）

> 研究档案/ 是 creative Phase 1 种子展开法的产出，v6.0 起从内部中间产物升级为下游可消费的正式接口文件。

| 研究档案文件 | 下游消费者 | 消费字段 | 用途 | 门禁 |
|:---|:---|:---|:---|:---|
| 域DNA档案-{域名}.md | world | 美学基因 + 经典元素库 | L1 设定的命名/质感/力量美学方向参考 | 跨域融合项目必读 |
| 创意溯源记录.md | character | 素材原型 + 拟用转化手法 | 角色卡设计的身份张力和说话风格参考 | Lv4 角色必读 |
| 交叉授粉.md | plot | 跨域联动方向 + 连接点类型 | L3/L4 剧情线设计的跨域联动参考 | 跨域融合项目必读 |
| 种子展开图.md | world, plot（可选） | 种子清单 + 切向扩展树 | 全局研究上下文参考 | 按需读取 |
| 研究档案索引.md | 全部下游 | 文件清单 + 域清单 + 消费映射 | 导航入口，确定该读哪些文件 | 研究档案/ 存在时必读 |
...
│
├── 研究档案/                            [creative] 种子展开法深度调研产出（S→D，下游 world/character/plot 消费）
│   ├── 研究档案索引.md                  [creative] 导航+消费映射+域清单
│   ├── 种子展开图.md                    [creative] 种子清单+直接研究+切向扩展树
│   ├── 域DNA档案-{域名}.md             [creative] 每域四维DNA+经典元素库 → world 消费
│   ├── 创意溯源记录.md                  [creative] 素材原型+拟用转化手法 → character 消费
│   └── 交叉授粉.md                      [creative] 跨域联动矩阵+连接点 → plot 消费
│
├── 小说世界设定/                        [world]
│   ├── L1-01世界蓝图.md
│   ├── L1-02力量体系.md
