...
-creative  -reservoir -world    -character -plot     -chapter  -prose   -qa
  v4.4.0    v3.3.0    v2.0.1    v2.0.3     v8.3.0    v2.6.1    v3.8.0   v1.0.1
```
...
| reservoir v3.3.0 | PRD + 外部素材 | `素材储备池/{素材}.md`（剧情储备卡/设定储备卡） | D | world, plot | 剧情储备卡（元卖点+冲突引擎+剧情线发展链条8~20节点）；设定储备卡（设定内核+可衍生冲突空间，标注对应L1维度） |
...
| Skill | 入 | 出 | S/D | → | 用途 |
|:------|:----|:----|:---:|:---|:-----|
| plot v8.3.0 Step 0 设计意图 | PRD + 角色卡 + 项目总控 | 用户设计意图模型（内存） | — | plot Step1 | 主线大类确认（世界大事件/主角成长/主角行动）+ 卷节奏+爽点优先级+角色弧线 |
| plot Step 1 卷战略 | `step-1-volume-strategy.md` + `volume-outline.md` | `剧情设计/卷/卷{N}-卷纲.md` **(L4)** | S | plot Step2 | 卷定位+起终点+爽点设计+篇幅预算+线数预算 |
| plot Step 2 候选线产出 | `step-2-seed-pull.md` + 储备池 + library | `卷/卷{N}-卷纲.md`（追加） | D | plot Step3 | 候选线雏形≥卷纲预算数量（储备池+库不够自己生成） |
| plot Step 3 剧情线成文 | `step-3-plotline-docs.md` + `plotline-doc.md` | `剧情设计/剧情线/{主线,支线,设定线}-{编号}-{名称}.md` **(L3)** + 回填卷纲剧情线总览 + `状态/state-log.yaml`（baseline #0） | S/D | chapter | 主线大类/具体剧情线/剧情节点三层结构；剧情发展骨架；创建 state-log baseline #0（弧线+伏笔+世界状态） |
| plot Step 4 分幕 | `step-4-act-plan.md` + `act-outline.md` + 全部剧情线 | `剧情设计/幕/vol-XX/act-YY.md` **(L2)** | S/D | chapter | 幕定位+活跃线+篇幅预算 |
...
| Skill | 入 | 出 | S/D | → | 用途 |
|:------|:----|:----|:---:|:---|:-----|
| chapter v2.6.1 | L2幕纲 act-YY + L3剧情线 + state-log.yaml | `章节设计包/chXXX-设计包.md` **(L1)** | D | prose | 回合级事件骨架；CH1补充state-log baseline #0角色状态；不写state-log（prose负责） |
...
| L2 剧情单元卡/幕纲 | plot Step4~6 产出的 `act-YY.md`（幕纲=单元卡超集：含Canvas/章锚点/债务追踪） | volume 产出的 L2单元卡（可迁移部分） | 超集关系 — 写作幕纲是拆书单元卡的前向设计扩展 |
| L3 剧情线 | plot Step3 产出的 `剧情线/{主线,支线,设定线}-*.md` | volume 产出的 L3剧情线卡（六段格式） | 同维度 — 写作正向设计，拆书逆向提取 |
| L4 全书事件/卷纲 | plot Step1~3 产出的 `卷{N}-卷纲.md` | volume 产出的 L4全书事件卡 | 同维度 — 写作按卷规划，拆书按卷提取 |
...
│   ├── 剧情线/                          [L3 剧情线]
│   │   ├── 主线-{编号}-{名称}.md        [plot Step3] 主线大类下具体剧情线，≥12节点
│   │   ├── 支线-{编号}-{名称}.md        [plot Step3] 无主线大类归属，≥5节点
...
> 写作专家 skill 清单：expert-writer(v4.9.0)、pop-writer-creative(v4.4.0)、pop-writer-reservoir(v3.3.0)、pop-writer-world(v2.0.1)、
> pop-writer-character(v2.0.3)、pop-writer-plot(v8.3.0)、pop-writer-chapter(v2.6.1)、pop-writer-prose(v3.8.0)、pop-writer-qa(v1.0.1)、
