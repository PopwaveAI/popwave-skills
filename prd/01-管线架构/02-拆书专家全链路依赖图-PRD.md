...
> **v2.1 迭代**：入库协议从"四模块"升级为"五库"，对齐 `pop-trope-library` 实际文件分类（立项库/设定库/文风库/剧情库/套路库）。设定库从"框架+质感"改为"按书整包"。文风库新增 canonical 路径，文风DNA 降级为 fallback。剧情库主条目从"章级剧情单元卡"改为"标准剧情线"。
...
| 模块 | 内容 | 服务对象 | 入库来源 |
|:-----|:-----|:---------|:---------|
| `立项库/` | PRD 级经验：立项模式/元爽点组合/题材焊接/加工哲学/失败案例 | creative, expert-writer | Phase 4 trace（SOP转化）+ 写书复盘 |
...
| `剧情库/` | **标准剧情线**（六段格式），按内容标签分目录；卷/幕切片仅做 legacy 证据 | plot | Phase 2 volume（剧情线提炼） |
...
| 管线阶段 | 入库模块 | 入什么 | 质量门槛 |
|:---------|:---------|:-------|:---------|
| Phase S 快速扫描 | `立项库/`（可选）| Lv1-拆解摘要中的立项级发现 | 已标注"基于前N章"，非强制入库 |
| Phase 1 design-pack | `原始素材/` | L3 具体套路卡（8字段齐全） | 触发条件/套路公式/核心张力/情绪收益/运作机制/使用注意/下游期待/复用记录 |
| Phase 2 volume | `剧情库/{标签}/{书}-{编号}-{名称}.md` | 标准剧情线卡（六段格式：线型/核心驱动力/阶段推进/叙事债务/可迁移边界） | 剧情线提炼核查清单通过；❌不按卷/幕切片直接入库 |
...
│       └── chekhov-tracker.md
│
├── 写作资产/剧情单元/                      [pop-decon-volume Step 6]
│   └── {单元名}.md                        → 提炼后入库 `剧情库/{标签}/{书}-{编号}-{名称}.md`
│
