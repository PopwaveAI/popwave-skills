...
| skill | 版本 | 定位 | 输入 | 产出 |
|:--|:--|:--|:--|:--|
| pop-fanqie-seed | v13.1.0 | 创意到首章 | 用户一句话方向 | `0-立项/创意.md` + `2-正文/ch001.md` |
| pop-fanqie-plot | v1.2.0 | 创意到完整故事世界 | 创意.md + ch001 | `1-骨架/骨架.md` + `剧情白描.md` + `章锚点表.md` |
...
| pop-dna-style | v1.0.0 | 文风 DNA 提取（笔触供给方） | seed 之前或 write 之前 | `涌现/文风锚定.md`（笔触DNA）+ 可选 `涌现/剧情DNA-brief.md` |
...
```
seed 产出 → plot 消费 → write 消费 → review 消费 → write 消费（下一章）
```
...
以下是一次完整首次运行的理论流程，从用户给出方向到写出第一章正文：

**Phase 0：前置供给（由 seed 1a 路由触发，非自动执行）**

seed 的 1a 问需求环节识别用户回答中的参考书意图，路由到对应供给环节。完成后回到 seed 继续 1b 市场调研。没有参考书需求时跳过 Phase 0。

| 用户回答 | seed 1a 路由 | 执行环节 |
|:--|:--|:--|
| "参考《深渊主宰》的笔触写一本 XXX" | → tool-download-webnovel 下载 → pop-dna-style 提取 DNA | 参考书下载+文风 DNA 提取 |
| "我想精拆《玄鉴仙族》做立项圣经" | → pop-decon Phase 1-4 精拆 | 拆书精拆 |
| "参考《深渊主宰》设定，精拆+提取笔触" | → download → decon + dna-style 并行 | 下载+拆书+DNA |
| 无参考书需求 | 跳过 Phase 0 | 直接进 1b 市场调研 |

Phase 0 的具体步骤（当用户触发时）：
...
| 横切环节 | 触发时机 | 典型场景 |
|:--|:--|:--|
| pop-research（种子级） | seed Phase 1 发散前 | 用户给的赛道陌生，需要世界观+压力矩阵调研 |
...
| pop-dna-style | seed 之前或 write 之前 | 用户有参考书想提取文风 |
| pop-decon | seed 之前 | 用户想精拆一本书做立项圣经 |
| tool-download-webnovel | 任何需要参考书时 | 用户给书名但无本地文件 |

---

## 5. 关键设计决策

### 5.1 笔触 DNA 可插拔（v8.0.0 核心改动）
...
### 5.3 seed 纯自由发散 + 市场校准（v13.1.0）

seed v13.1.0 恢复了 v6.0.0 的纯自由发散核心（R9 验证：纯自由模式产出的创意质量远高于注入搜索数据/格式模板的模式），同时补回了 R13/R14 的市场调研环节：发散前 WebSearch 3 轮产出借鉴点/避雷点清单，发散后用清单做事后校准。这兼顾了创意自由度和市场贴合度。
...
| skill | 版本 | 核心改动 |
|:--|:--|:--|
| pop-fanqie-seed | v13.1.0 | 补回市场调研+纯自由发散核心 |
| pop-fanqie-plot | v1.2.0 | 四图叠加推演剧情白描 |
| pop-fanqie-write | v8.0.0 | 减法版，移除 8 项内置风格倾向，笔触层移交 DNA |
| pop-fanqie-review | v4.1.0 | 同步 write v8.0.0，技法落地改笔触 DNA 一致性 |
| pop-dna-style | v1.0.0 | 新建，笔触 DNA 供给方，双档位+场景定向采样 |
| pop-research | v1.2.0 | 新增采风级档位，write 加入调用方 |
