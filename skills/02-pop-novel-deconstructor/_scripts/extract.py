...
def extract_index(txt_path, output_dir):
    ...
    seen_nums = set()
    for c in chapters:
        if c[0] not in seen_nums and 1 <= c[0] <= 100:
            ch1_100.append(c)
            seen_nums.add(c[0])
    ...
    for idx, (num, start, title) in enumerate(ch1_100):
        ...
        if idx + 1 < len(ch1_100):
            ...
            next_ch = next((c for c in chapters if c[0] > 100), None)
...
def extract_world(txt_path, output_dir):
    ...
    for i, line in enumerate(lines):
        ...
        if m:
            num = int(m.group(1))
            if 1 <= num <= 100 and num not in seen_ch_nums:
                current_chapter = num
                seen_ch_nums.add(num)
            elif 1 <= num <= 100 and num in seen_ch_nums:
    ...
    if ch100_end is None:
        first_beyond = next((c for c in chapters if c[0] > 100), None)
        ch100_end = first_beyond[1] if first_beyond else len(lines) - 1
    else:
        # Scan ends at the chapter AFTER ch100's first occurrence
        ch101 = next((c for c in chapters if c[0] > 100 and c[1] > ch100_end), None)
        if ch101:
            ch100_end = ch101[1] - 1
