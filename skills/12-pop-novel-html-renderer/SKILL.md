...
# html-renderer — HTML 化发布引擎 v1.3.1
...
> - **pop-html-anything** → 通用 HTML 渲染引擎，消费任何上游结构化数据
...
| **渲染通用 HTML** | 非 pop-novel-master 场景 | ❌ 不适用 — 用 `pop-html-anything` |

### 判断指引

| 场景 | 用哪个 |
|:-----|:-------|
| 拆书报告需要 HTML 发布页 | `pop-html-anything` |
...
| ❌ 门禁 | `pop-html-anything` 未安装 → 抛出 `ImportError`，提示安装后重试 |
...
| # | 场景 | 触发条件 | 处理动作 |
|:-:|:-----|:---------|:---------|
| 1 | pop-html-anything 未安装 | 渲染器初始化时无法导入 pop-html-anything 模块 | 抛出 `ImportError`，提示「请先安装 pop-html-anything SKILL」并中止渲染 |
...
| 7 | pop-html-anything 版本不兼容 | 已安装但版本过低，缺少本 renderer 依赖的 API | 提示用户更新 pop-html-anything 至兼容版本，显示当前版本和最低版本要求 |
...
v1.3.1 | 2026-06-04 | 完整变更记录 → [CHANGELOG.md](CHANGELOG.md)
