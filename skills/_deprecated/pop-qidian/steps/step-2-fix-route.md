...
本 step 的路由建议是**一次性诊断建议**，非每轮正文调度总控。正文写作轮次由用户或 PE 提示词直接调用 pop-qidian-write。

路由建议表（参照 PRD §7）：

| 情况 | 下一步 |
| --- | --- |
| 没有 seed 或主卖点不清 | `pop-qidian-seed` |
| 缺现实/题材/机制燃料 | `pop-qidian-research` |
| 缺 current-state 或 current-state 过期 | `pop-qidian-review` 初始化或修复 |
| 缺 soul 或 soul 空泛 | `pop-qidian-review` 提出 soul 修复，用户确认后更新 |
| 启用文风DNA但缺执行包 | `pop-qidian-review` 把下一章DNA执行包压入 current-state |
| 缺章内文风DNA源 | `pop-shared-dna` 从参考小说产出章内DNA |
| 入口文件齐全且要写正文 | `pop-qidian-write` |
| 写完一章 | `pop-qidian-review` 更新 current-state |
