...
### P1优化：设计包范式转换——事件链→场景流

- **fact-skeleton.md**：事件字段16→7核心字段；DQTC字段移到独立"决策溯源附录"；自检表23项→5项；调味禁忌→创作方向建议；信息释放简化
- **step-2-event-chain.md**：从"逐事件设计"→"场景流设计"（3-5场景/章，每场景3-5事件有机流动）；门禁从事件数→场景数
- **step-3-polish.md**：子表8→3（爽点标注/情绪弧线/章末钩子）；契诃夫枪表→state-log；调味空间→创作方向建议
- **大版本升级**：设计包范式从"逐事件填表"→"场景流设计"
