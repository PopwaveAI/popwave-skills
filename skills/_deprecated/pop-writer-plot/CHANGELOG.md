...
- **step-6-canvas-audit.md §一**：新增"每章≥8个事件"+"每3章≥1个完整情绪闭环"密度基准
- **step-6-canvas-audit.md §七**：新增"危机来源多样性检查"——每幕危机≥3系统，无连续3章同源
...
**问题1**：Step 3 门禁写死"主线超过1条→默认退回合并"，与 Step 0 用户确认的多主线结构直接矛盾。东方版深渊主宰项目中，用户在 Step 0 确认了3条主线，但 Step 3 门禁强制合并成1条，导致全链结构错配。

**问题2**："世界危机/主角成长/主角行动"被错误定义为三条主线，实际上它们是三个主线大类（L1），不是具体剧情线（L2）。

**问题3**：L2 被错误理解为全书固定 2-3 条，实际上 L2 是每卷动态激活的——每卷每个 L1 大类下有 2-3 条 L2 同时推进。

**问题4**：支线被错误要求挂 L1 大类，实际上支线合不进三大类，直接是 L2 级别，无 L1。

**问题5**：plotline-doc.md 剧情发展骨架只有6行空表，120章卷需要主线L2≥12节点、支线L2≥5节点。

**问题6**：Canvas 密度"低/中/高"没有量化定义。

**改动**：
- `steps/step-3-plotline-docs.md`：新增「剧情线三层结构」——L1全书固定/L2每卷动态2-3条/L3节点；新增「主线vs支线层级差异」——主线L1→L2→L3，支线L2→L3无L1；L1"世界危机"改名"世界大事件"；L1→L2动态绑定规则（每卷每大类2-3条，可跨卷）；示例结构改为按卷展示；门禁改为"某卷某L1大类下L2少于2条"
- `SKILL.md` Step 0：同步三层结构+世界大事件+支线无L1+动态L2
- `templates/volume-outline.md`：线数配置表改为每卷每大类2-3条L2（标准卷8-12/长卷12-15）；底线改为"每卷每个L1大类至少2条L2活跃"；线数决策表拆为3行（每个L1大类各1行）+支线标注"无L1"
- `templates/plotline-doc.md`：来源表新增"所属L1大类"字段；剧情发展骨架从6行改为按线型区分（主线≥12节点/支线≥5节点/设定线≥3节点）
- `templates/act-outline.md`：活跃线表新增"所属L1大类"列；Canvas矩阵新增密度量化定义（高≥4线/中2-3线/低1线）；篇幅预算从2.5k改为2~3k
- `steps/step-4-act-plan.md`：活跃线描述对齐L1大类；篇幅预算2.5k改为2~3k

**效果**：L1/L2/L3 三层结构清晰区分"主线大类"和"具体剧情线"。储备卡对应L2，不再混淆。成长线和行动线有了可量化的验收标准。
