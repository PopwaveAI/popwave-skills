...
### P0+P1优化：不解释原则修正+场景流渲染

- **P0 不解释修正**：step-1-render.md "不解释"→"限制内省"（每章1-2处内心活动，80%展示+20%内省）
- **P0 DNA红线**：SKILL.md 红线#2 从"A/B选项给用户"→"硬阻塞终止"
- **P1 场景流渲染**：step-1-render.md 从"逐事件渲染"→"场景流渲染"（场景内连续流动，不逐事件独立成段）
- **同步更新**：设计包读取从"事件链"→"场景流"，红线/门禁同步
