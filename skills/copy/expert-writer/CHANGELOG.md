...
## [9.10.0] - 2026-06-29
### Changed
- step-2-0导演意图提取：输入源从幕纲"结构分析表"改为"创作节拍表"（配合pop-writer-v3-plot v1.4）
- director_intent YAML新增unit_redlines字段（单元红线）
- 导演意图组装规则从提取表格列改为提取节拍表字数段动作链合并

## [9.9.0] - 2026-06-28
### Changed
- 管线/执行分层解耦：管线段不再把expert-writer列为阶段（改为"写作(expert-writer调度)"）；原"执行流程"段改为"请求处理"表格，明确Execute按路由执行不写死5步循环
- 5步循环门禁标注作用域："仅写第X章/继续/下一步时运行"
- 网文铁律8条精简文笔：去嵌套从句，每条压缩到1-2句核心判断标准
- 红线❌8描述精简
### Removed
- "两个人工check点"段+弧线触发段移除：执行细节已在step文件中（step-2-0第6行+step-2-3第6行），调度层不重复

## [9.8.0] - 2026-06-28
### Changed
- SKILL.md压缩优化：183行→105行（省43% token）
- 删除：管线模式声明表格（单模式无需表格）/ 速查表全文件目录（agent按需读step文件）/ 核心流程段（重复管线顺序）/ 版本历史（CHANGELOG已有）/ 种子文档+素材库历史迁移说明
- 管线声明表格→1行 / library查询矩阵6行→1行 / 文件加载规范5行表格→1行 / 路由表合并为1个
### Added
- ❌6❌7合并升级为「网文铁律8条通用原则」内联：读者优先/默认快节奏/爽感驱动/主角主动/危机不间断/行动即回报/持续悬念/章末追读
- 区分原则（SKILL.md内联）与技法（create创作指南引用），原则是所有网文通用的底层规律，技法是量化标准和实现方式
### Removed
- ❌6（网文写作铁律：直接易懂爽感优先）— 升级为网文铁律8条
- ❌7（世界观卖点前置）— 升级为网文铁律第7条「持续悬念」，覆盖全书而非仅开篇

## [9.7.0] - 2026-06-28
### Added
- 全文注入铁律红线（❌8禁止摘要注入子agent）
- manifest.md新增「全文注入铁律」章节：禁止主agent提炼/摘要后注入，所有文件类注入项必须全文
- create manifest新增幕纲全文注入+文风DNA全文注入（仅参考）
- revise manifest新增幕纲嵌套子线全文注入+文风DNA全文注入
- receipt新增injection_verified字段（确认收到的是全文不是摘要）
- receipt检查从6项扩展为7项（新增幕纲全文注入验证）
### Changed
- step-2-3 dispatch文件全面重写：manifest_version 1.0→1.1，所有注入项新增injection: full标注
- create manifest注入项从6项扩展为8项（新增幕纲全文+文风DNA全文）
- create receipt从three_questions确认改为five_questions确认
- revise manifest的element_slices改为l2_sublines（幕纲嵌套子线全文）
...
### v3.5涌现式写作管线重构：6步→5步+幕纲替代种子文档+context manifest白盒
...
| 循环步数 | 6步 | 5步 |
| 活文档 | 种子文档.md | 幕纲 |
| 种子文档 | 存在 | 取消（功能被幕纲+写作参考吸收） |
| 结构约束 | 种子文档任务表 | 幕纲（跨章）+导演意图（单章） |
| 设定读取 | 靠agent自觉 | 幕纲设定指针→主会话强制读→manifest验证 |
| 状态管理 | 活记忆7组件结构化YAML | 活记忆自然语言段落 |
| 子agent上下文 | 黑盒 | context manifest白盒 |
| 素材库+设定库 | 分开 | 合并为写作参考/ |
| arc触发 | 每10-20章 | 每个L2单元结束时 |
| 种子生长 | 8种场景 | 删除（幕纲每单元由arc更新） |

**5步循环结构（替代6步）：**
- Step0：导演意图提取（从幕纲结构分析表取本章行→组装导演意图≤150字）
- Step1：状态快照投影（从活记忆+幕纲物理坐标投影≤400字）
...
**文件变更：**
- `SKILL.md`：v9.4.0→v9.5.0，6步门禁表→5步门禁表，5条红线更新，文件加载规范更新（幕纲+写作参考替代种子文档+素材库+设定库），管线顺序新增plot+pop-research，emerge标注废弃
- `skill.json`：v9.4.0→v9.5.0，downstream新增pop-research
- `steps/step-0-init.md`：项目空间结构更新（卷纲/L2/L3+写作参考替代素材库+设定库）
- `steps/step-1-think.md`：管线锚定更新（幕纲替代种子文档），5步循环路由
...
- `steps/step-3-reflect.md`：更新引用（5步循环+幕纲）
- `references/pipeline/manifest.md`：v3.3→v3.5，阶段表新增plot和pop-research
- `references/project/master-control.tpl.md`：管线身份更新为v9.5.0，种子文档引用改为幕纲
