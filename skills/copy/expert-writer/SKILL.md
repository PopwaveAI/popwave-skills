---
name: expert-writer
description: "当用户说'开书/拆书/设计剧情/写正文/审稿/继续/下一步/回滚'时启用。自动路由到对应子Skill。v3.5涌现式写作管线唯一调度引擎，幕纲驱动。涌现写作环5步循环由主会话直接执行。"
version: 9.10.0
---

# expert-writer · 写作专家调度引擎 v9.10.0

> 网文创作元 Skill（唯一调度器）。Think → Execute → Reflect 三层工作流。v3.5涌现式写作管线专用。幕纲为唯一运行时活文档（v1.4导演指令集格式）。

## 项目管线（书级SOP）

种子设计(v3-seed) → 幕纲设计(v3-plot) → 写作(expert-writer调度) ↔ 弧线校准(v3-arc)

> pop-research按需调用。管线合同详见 references/pipeline/manifest.md。
> 不走大纲/章纲。叙事结构由"幕纲结构分析表（跨章）+导演意图（单章）+设定引用指针（设定穿透）"三角约束驱动。
...
| ❌8 | **禁止摘要注入子agent（全文注入铁律）** — 所有文件类注入项必须全文注入。幕纲/设定文件/文风DNA/create初稿全部全文。摘要丢失信息且不可控——比不注入更危险 |
...
| 步骤 | 动作 | 硬门禁 | 验证证据 |
|:--|:--|:--|:--|
| Step0 导演意图提取 | 从幕纲创作节拍表取本章表→组装导演意图（≤150字） | 导演意图含五问+worldview_delivery+settings_ref+unit_redlines+用户确认 | director_intent YAML（含narrative_function/event_chain/emotion_curve/five_questions/worldview_delivery/settings_ref/unit_redlines） |
| Step1 状态快照投影 | 从活记忆最新events+幕纲物理坐标投影当前状态（≤400字） | 状态快照含protagonist+pressures+pending | state_snapshot YAML（不持久化，每章实时投影） |
...
| Step4 receipt检查 | 对照manifest vs receipt→对照导演意图验证→全文注入验证 | 完整性+关键元素+导演意图+设定全文+文风DNA全文+幕纲全文+6项验证全部通过 | receipt一致性检查结果（7项） |
...
| 文件类型 | 加载方式 |
|:--|:--|
| 文风DNA / 幕纲 / 写作参考索引 / 设定文件 / 大文件(>10KB) | 一律 `Get-Content -Encoding UTF8 -Raw` 完整加载 |
...
| "设计剧情/幕纲" | pop-writer-v3-plot | seed已完成 |
| "继续/下一步/写第X章" | expert-writer(5步循环) | 幕纲已产出 |
...
> pop-trope-library查询：seed(套路库+金手指库) / plot(剧情库幕纲) / revise(文风库) / arc(剧情库幕纲)。
