...
| `卷纲/幕NNN-名称.md` 不存在 | 种子设计 | **pop-writer-v3-seed** |
| 幕纲已产出，`正文/ch001.md` 不存在 | 幕纲设计 | **pop-writer-v3-plot** |
| 幕纲已产出，`正文/` 有文件，但L2单元未写完 | 涌现写作 | **expert-writer 5步循环**（本skill Execute阶段） |
| L2单元最后一章 Step5 完成 | 弧线校准 | **pop-writer-v3-arc** |
| 用户说"回滚" | 回退 | `step-2-execute.md` §3.2 |
| 用户说"调研/查资料" | 按需调研 | **pop-research**（按需调用） |

> **关键变化**：
> - 原判断"种子文档是否存在"→ 改为"幕纲是否存在"
> - 原判断"种子文档任务表"→ 改为"幕纲嵌套子线"
...
| "设计剧情"/"幕纲" | 幕纲设计 | pop-writer-v3-plot |
