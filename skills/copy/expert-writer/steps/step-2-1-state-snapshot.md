# Step1：状态快照投影 — 从活记忆+幕纲物理坐标实时投影当前状态

> **执行者**：主会话
> **输入**：活记忆（`活记忆/活记忆.yaml`）最新events + 幕纲物理坐标段
...
1. `Get-Content -Encoding UTF8 -Raw` 读取 `卷纲/幕NNN-名称.md`
2. 定位 **物理坐标段**（幕纲中记录当前物理状态的段落）：
   - 主角当前能力/属性
   - 主角当前心理状态
   - 当前所处环境/位置
   - 当前可用资源
...
> 幕纲物理坐标段是结构化的状态记录，与活记忆的自然语言日志互补。
> 活记忆提供叙事状态变化，幕纲物理坐标提供精确数值/状态。

---

## 3. 投影状态快照

从活记忆最新events + 幕纲物理坐标段，实时投影当前状态（≤400字）：
