...
> **输入**：导演意图 + 状态快照 + 幕纲全文 + 设定文件全文 + info_acquired + 文风DNA全文 + create初稿全文
...
- create阶段：注入导演意图+状态快照+幕纲全文+上章末尾+设定文件全文+info_acquired+文风DNA全文（仅参考） → 产出初稿+receipt
- revise阶段：注入初稿全文+文风DNA全文+幕纲嵌套子线全文+导演意图验证清单 → 产出重写稿+receipt
- 两阶段自动连贯，中间不暂停
...
context_manifest:
  ...
  items:
    ...
    - id: l2_card_full             # v9.7新增：幕纲全文注入
      type: text
      source: "卷纲/L2-001-名称.md"
      read_method: "Get-Content -Encoding UTF8 -Raw"
      size: 14000
      description: "幕纲全文（唯一运行时活文档，子agent从中提取本章行）"
      injection: full              # 禁止摘要！幕纲替代种子文档，必须全文
