...
creative  → reservoir  →  world  →  plot v7.0  →  chapter v2.1  →  prose  →  qa
 ↑            ↑               ↑
Phase R      剧情储备卡格式   L1-01~07+角色+数值
→爽点引擎    （9步注入）      +起点快照+宪法
→A/B/C
→PRD
→层架构.md
...
|:----:|:------|:-----|:---------|:-----------|
| 1 | pop-writer-creative | 爽点引擎+PRD+层架构.md+故事引擎+样品 | 无 | creative 产出全部到位? |
| 2 | pop-writer-reservoir | 剧情储备卡(含安全门禁通过) | PRD+故事引擎已产出 | 安全门禁已执行? 储备卡格式完整? |
| 3 | pop-writer-world | L1-01~07(含L1-07术语与文明底色)+角色+数值+宪法+升级表 | creative+reservoir 已产出 | world 落盘检查点全通过? |
...
| "设计第X章/章纲/骨架" | pop-writer-chapter (v2.1 升级) | 必须先完成 plot v7.0(剧情线设计文档 + 幕锚点 + chekhov-tracker) | 不纠结渲染用词（那是 prose-render 的活）。设计包含情绪弧/爽点机制/钩子回收/契诃夫枪/对白潜台词/★调味空间 |
...
- **项目总控.md 已更新**（📊首屏仪表盘 + 🗺️SOP全流程 + ⚠️管线完整性警告 + 🚀下一步 + 产出物公示表 + 📁文件树 + 待处理风险）
...
### 集中合同优于分散声明（2026-06-19 项目B根因沉淀）

当涉及**多个组件之间的执行顺序、数据依赖、阶段划分**时——不要依赖每个组件各自声明自己的 `upstream/downstream` 字段来推导全局顺序。单个组件的 pipeline 声明只能表达"谁是我邻居"，无法表达全链路顺序。Agent 读任何一个组件的声明都会得到不完整的顺序图。

**正确做法**：定义一个中心化合同文件，硬性规定全链路顺序，所有组件和 Agent 都查这份合同。组件级的 pipeline 字段只用于描述"我消费谁/谁消费我"的数据依赖——不用于推导执行顺序。

| 场景 | 错误做法 | 正确做法 |
|:-----|:---------|:---------|
| 定义管线阶段顺序 | 分散在各 SKILL.md 的 `upstream/downstream` 字段 | 一份 `pipeline-manifest.md` 硬性规定顺序 |
| 检测当前进度 | 从各子 skill 的文件存在性推断 | `项目总控.md` 记录执行顺序日志 + 管线进度标记 |
| 产出物质量检查 | 各阶段独立产出，无全局对账 | 健康检查用产出物公示表做全链路一致性审计 |

项目B事故的直接根因就是这个模式问题：Agent 看到 world 的 `upstream: [pop-writer-creative]` 就认为 creative→world 是正确顺序，跳过了 reservoir。不是 Agent 不仔细——是信息本身就不完整。

**核查**：当用户提出"我的管线有断裂/顺序错了"时——**先问"哪里是信息的单一真相源"**，而不是问"哪个组件的声明要修"。

## 版本

v3.5.0 | 2026-06-22 | 项目总控模板 v2.0：新增 📊首屏仪表盘（卡片网格+项目统计）、🗺️SOP全流程（T1-T7展开+每步一句话）、⚠️管线完整性警告（诚实暴露跳跃）、🚀下一步（具体动作清单替代抽象阶段名）。📁文件树改为理想+实际双轨（🔧标记非标准路径）。output-formula-design.md 升级为完整设计说明。
v3.4.1 | 2026-06-19 | 同步子 skill 版本：creative v3.1.0(新增层架构.md产出) / world v2.0.0(L1-01~07含术语模块+按层重组Phase1) / chapter v2.1.0(设计包新增调味空间字段) / reservoir v2.3.0(强制读取设定库)。pipeline-manifest+pipeline-arch 同步更新产出清单。
v3.4.0 | 2026-06-19 | 新增第三核心原则"集中合同优于分散声明"；新增产出物公示表与PRD映射参考。详见本行展开。
| 子项 | 改动 |
|:-----|:------|
| pipeline-manifest.md | 新建，管线硬顺序合同+截断检测协议 |
| project-master-control.tpl.md | 新建，含管线进度+执行顺序日志+产出物公示表(A+B+C=D)+理想文件树+风险 |
...
| output-formula-design.md | 新建，产出物公示表设计说明，映射 PRD §3.x 依赖表 |
