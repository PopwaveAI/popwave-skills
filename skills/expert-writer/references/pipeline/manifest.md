...
| 5 plot | pop-writer-plot | v9.0.0 | `剧情设计/卷/L4-{编号}-{事件名}.md`（L4全书事件） + `剧情设计/卷/卷{N}-卷纲.md`（每卷战略） + `剧情设计/剧情线/L3-{编号}-{名称}.md`（L3剧情线） + `剧情设计/幕/vol-XX/L2-{编号}-{单元名}.md`（L2单元卡） | world+character 产出齐全 + trop-library 已查 | 里程碑确认 | 套路库/ + 剧情库/ + 元爽点-变体映射表 |
| 6 chapter | pop-writer-chapter | v2.7.0 | `章节设计包/chXXX-设计包.md` + state-log读取（取before状态） + L2-{编号}读取 | plot 产出齐全 | — | 套路库/{具体套路名}.md |
