...
扫描实际文件系统，对照 `项目总控.md` 中的理想文件树逐项标记。结果直接更新到 `项目总控.md` 的 `## 理想文件树` 段，对话只输出摘要。

```
执行方式：
1. 按理想文件树（项目总控.md §理想文件树）的每条记录搜索对应文件
2. 文件存在 → 标记 ✅（已存在于项目总控.md）
3. 文件不存在但对应阶段已完成 → 标记 ❌ 缺失
4. 文件不存在且对应阶段未开始 → 标记 ⬜ 待产出
5. 更新项目总控.md 中的文件树标记
6. 计算统计：应有 N 个 / 已有 M 个 / 完成度 M/N×100%
