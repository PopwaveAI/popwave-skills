...
**1b — 产出物完整性（文件树 + 产出公示表双比对）**

扫描实际文件系统，同时更新项目总控.md 中的两个全貌段。对话只输出摘要。

```
执行方式：
1. 按理想文件树（项目总控.md §理想文件树）的每条记录搜索对应文件
   文件存在 → 标记 ✅ | 不存在且阶段已完成 → ❌ | 不存在且阶段未开始 → ⬜
2. 按产出物公示表（项目总控.md §产出物公示表）逐行扫描：
   该行的「产出」文件是否存在 → 更新状态列 ✅/⬜/❌
3. 计算统计：应有 N 个 / 已有 M 个 / 完成度 M/N×100%
4. 更新项目总控.md 中的标记
5. 如果某个产出文件缺失但其所有输入文件都存在（说明产出者偷懒了）→ ⚠️ 标记
