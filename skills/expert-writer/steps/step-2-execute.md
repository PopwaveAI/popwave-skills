...
**v3-emerge 执行时**（3子agent调度）：

1. **信息获取调度**（Step 1 强制化）：提醒 emerge 读取 `写作资产/知识库/索引.md`（强制），有匹配→读知识库文件，无匹配→WebSearch→写入知识库→更新索引。门禁：索引未读取=退回补读。

2. **创作子agent**（Step 2，context隔离）：传入精简context（种子六要素+活记忆+上章末尾+chapter_plan+info_acquired+创作模板），不传会话历史。任务=纯故事涌现（场景流+压力源+章末钩子），不管文风。输出=正文初稿+创作决策记录。

3. **修订子agent**（Step 3，context隔离）：传入初稿+文风DNA+种子六要素+活记忆+修订checklist。任务=文风对齐/人设丰富/爽点验证/bug修复/AI观感词清理。输出=修订稿+修订记录。

4. **质检子agent**（Step 4，context隔离）：传入修订稿+活记忆+种子六要素+网文法则+质检模板。任务=五问反思(引用正文证据)+种子生长判断+爽点终验。输出=质检报告+通过/不通过+回退目标。不通过→回退Step 2(故事层)或Step 3(文风/人设)。
...
| 批量写N章/风格迁移 | 管线旁路：跳过正常涌现写作环，直接从源文本提取事件链 → 委托创作子agent并行渲染。批次划分/创作context由emerge自管 |
