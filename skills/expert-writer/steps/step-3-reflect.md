...
□ **管线进度更新**：
  ...
  - **回写项目总控.md（上半屏 📊 项目现状）**：
    - 找到项目根目录的 `项目总控.md`
    - 更新 `📊 项目现状 → 首屏仪表盘`：将刚完成的阶段标记从 ⏳/⬜ 改为 ✅
    - 更新 `📊 项目现状 → 实际阶段执行`：追加已完成阶段行（或更新已有行状态）
    - 更新 `📊 项目现状 → 产出物清单`：扫描文件系统，更新状态列
    - 更新 `📊 项目现状 → 当前进度锚点`：同步 entity-snapshot.yaml 的章号/幕/主角状态
    - 扫描文件系统更新 `📊 项目现状 → 目录结构`（如有新文件/新目录）
    - **追加执行顺序日志**：新增一行 `| {序号} | {阶段名} | {当前时间戳} | {备注} |`
    - 更新 `待处理/风险`：新增风险或删除已解决的条目
    - **下半屏 🗺️ 所属管线不动**（管线定义不变，只有版本漂移时才刷新）
  - **首次初始化时**（项目总控.md 刚以模板创建时）：
    - 从文件修改时间戳推算出已有阶段的执行顺序，补全执行顺序日志的前序记录
    - 扫描实际文件系统，填充 `📊 项目现状` 全部状态标记和目录结构
  - **存量项目首次健康检查时**（项目总控.md 不存在）：
