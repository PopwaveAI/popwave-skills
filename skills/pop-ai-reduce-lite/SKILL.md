...
> **减法只做在「虚」的地方**：模板化连接词、空洞升华、机械对称句式，一律删去；原作的口语表达、具体细节、个人语气、感官描写，一律保留。宁可留着粗糙，不削成骨架。

**三条铁律**：

1. **立场守恒**：不改变原文的事实陈述（时间、地点、事件、数据）、价值判断（褒贬倾向）、情感极性（悲、喜、怒、平淡）。违反任一条即视为执行失败。
...
| 强度 | 匹配场景 | 特征 |
|------|---------|------|
| **保守** | 学术论文、技术文档、法律文书、政务文件 | 仅删 AI 连接词和模板套话，不改句式结构，不加入口语 |
| **标准**（默认） | 自媒体、职场汇报、日常沟通、公关稿件 | 完整去 AI 味清单，适度口语化，保留原文核心结构 |
| **激进** | 创意写作、小说、散文、社交媒体帖子 | 可大幅调整视角、句式、段落，加入强烈的个人语气 |

### 1.3 文本规模自适应

| 字数 | 策略 |
|------|------|
| **< 1000 字** | 完整四步流程，诊断结果显式输出 |
| **1000-3000 字** | 合并诊断与改写，精简中间输出，保留质检 |
...
**脚本不可用时的兜底**：按以下 7 个维度逐项诊断（不得只给综合评分）：
...
输出格式为 `D{n}: [x]/[!] — 证据：xxx`，并附上风险定级（低、中、高）与改写方向。

> **D8 · 抽象对偶比喻**（AI 腔"看起来有深度、实际零信息"的典型结构）：
> 检测：抽象名词A（命/命运/世界/时代/历史/文明/人生/人性/压迫/真相/意义/欲望）接 联结词（是/像/如同/宛如/化作/成了）接 抽象陈词B（账本/记账/契约/迷宫/深渊/棋局/棋盘/天平/洪流/注脚/泡沫/答卷）。命中 → `[!]`。
> 判据三条：
> 1. **两端须具体可感**：至少一端是能看见或摸到的物（锈铁、锅底、老街、断梯）。两端都抽象时，删去或换成具体物。
> 2. **必须增加信息**：删掉这句，读者会少知道哪件具体的事？若只是"换皮不增加信息"（把压迫叫作账、把世界叫作迷宫），则改为直陈。
> 3. **贴合影象豁免**：喻体若与本作叙事真实贴合的核心意象一致（主角真在跟外神对弈，则可称"棋手""一步棋"），而非为升华而空，则保留，不判违规。
...
| 步骤 | 操作 | 方法 |
|------|------|------|
| **1 搜黑话** | 全局搜索 | 找「赋能/闭环/底层逻辑/深刻/交织/画卷/交响乐/总而言之/综上所述」。搜到一处，就用具体名词或动词替换一处。 |
| **2 切长句** | 拆定语 | 把所有「的」字超长定语（3 个「的」以上）从「的」字处切断。删「在……的过程中」「进行了……的讨论」等翻译腔框架。 |
| **3 加泥土** | 加入具象 | 在最干瘪处强行加入具体物品（「一杯冷掉的咖啡」）或微小动作（「叹了口气」）。每 500 字至少 1 处。 |
| **4 保血肉** | 止损检查 | 通读：口语、孩子气口吻、动作描写、场景细节、语气词，一个不丢。只砍「虚」的，不碰「实」的。 |
...
| **L5 破语气** | 让「我」的立场和情绪站出来 | 「可能是」改为「我赌五毛钱是」；加入小吐槽、怀疑、自嘲 |
...
| 升华结尾 | 以总结/展望/呼吁收尾 | 戛然而止，或以细节、疑问结束 |

### 2.4 风格注入规则

- **短句占比** >= 全文句数 30%
- **人味落点**：至少 1 处第一人称视角或主观感受
- **词汇降级**：利用改为用、进行购买改为买、具备能力改为能
- **标点呼吸**：适当使用破折号、省略号、问号制造停顿
- **细节补充**：每 500 字至少 1 处场景、数字或感官落点
...
识别不符合项，定位根因，回到步骤2修复，然后重新执行本清单。修复尝试上限 2 次，超限标记为"人工复核"。
...
| 需求冲突（如去AI味与保留原结构冲突） | 记录冲突，选择影响最小的兼容路径 |
| 质检不通过 | 回步骤2修复，上限 2 次 |
| 高风险变更（学术文体改为网文体） | 输出风险评估，拒绝执行 |
...
| `scripts/detect.py` | AI 痕迹检测脚本（15 类正则扫描） |
| `scripts/rewrite.py` | AI 文本改写脚本（8 步流水线） |
