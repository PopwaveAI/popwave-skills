...
body 检测面（2026-09-05 扩容，词条按内部语料统计汇总）:
...
body 阈值校准（2026-09-05）:
  ...
  破折号密度 WARN 1.0 / FAIL 2.0（/500字）
  省略号     WARN 8 / FAIL 12（/千字；对话尾……系作者风格）
  语气词/语气句/四字格连排/5字重复/段长CV/单引号 → REPORT_ONLY（风格方差过大）
  引号未配对 WARN 3/FAIL 6（跨行引文块合法）；烙印移出意象动词（奴隶烙印实体用法）
  en_line 需两个3+字母英文词（"mss！"单词感叹对白放行）
  原则：WARN 线设在自然波动上沿，FAIL 再上浮。
"""
import argparse
import json
import os
...
CHECKS = [
    ...
    ('mou_density',     '眸系描写',   2, 4, '/千字', '眸光/眸底/抬眸——高频眼部修辞，换直白眼神动作或删'),
    ...
    ('dun_dun',         '顿了顿',     2, 4, '/千字', '"顿了顿"是高频口癖——删掉多数，一章最多留一处'),
    ...
    ('gan_sense',       'X感词族',    1, 2, '/千字', '宿命感/破碎感/少年感/氛围感——抽象审美词，换成看得见的画面'),
    ...
    ('causal_density',  '因果解释',   5, 8, '/千字', '因为/由于/为了/试图/想要扎堆——删掉解释，让动作自己说'),
    ('nominal_v',       '名词化动词', 1, 2, '/千字', '进行了/做出了/完成了——直接写动词（进行了一场比赛→比了一场）'),
    ('er_start',        '句首而字',   3, 5, '/千字', '句子以"而"开头（而这一切/而他）——改直接陈述'),
    ('paren_note',      '括号注释',   7, 10, '/千字', '正文里（说明性括号）——删掉或改进叙述'),
    # --- 模板类（处） ---
    ...
    ('sent_cv',         '句长变异系数', 0.38, 0.30, '', '全章句长波动太小，穿插极短句和长句拉开方差'),
    ...
    ('ngram3_rep',      '三字片段重复率', 0.24, 0.30, '', '同一3字片段反复出现（机械重复感），换说法或删'),
    ('punct_cv',        '标点节奏变异', 0.30, 0.22, '', '标点间隔过于均匀，用长短句打破'),
    ('sem_smooth',      '顺滑句对占比', 0.15, 0.25, '', '相邻句高度相似，插跳跃、换话题或删'),
    ('ttr',             '二字组多样性', 0.66, 0.62, '', '用词多样性过低（翻来覆去那几个词），引入新词和具体名词'),
    ('tone_div',        '语气句比例', 0.15, 0.08, '', '问句/感叹句太少——对白里加反问和情绪句'),
    ('modal_part',      '语气词密度', 3, 1.5, '/千字', '对白里几乎没有啊/吧/呢/嘛——给台词加口气'),
    ('dunhao',          '顿号密度',   1, 0.3, '/千字', '仅报数：本项只统计顿号数量；方差大，不作判级依据'),
    ('func_word',       '的着了气口', 36, 28, '/千字', '的/着/了密度低于参照下限——补自然口语连接'),
    ('pron_density',    '代词气口',   5.5, 4, '/千字', '这/那/这个/那个密度过低——指代落地到人和物'),
    # --- 其他 ---
    ...
    ('sq_quote',        '单引号使用', 4, 8, '处', '少量‘’多为术语引用/内心独白惯例（正文惯例合法）；整章大量出现才可能是格式转换——按书内惯例判断'),
    ...
    # --- v3.3 新增（引号体系/分割线，2026-09-08）---
    ('quote_mixed',     '引号体系混用', 1, 3, '处', '“”与「」『』并存——全书统一一种引号体系'),
...
PAT_AI_META = (r'已思考|已深度思考|思考中|思考残留|用时\d+秒|嗯，用户|好的，用户|'
               ...
               # v3.3：知识截止免责/谄媚语气/客服短语（2026-09-08）
...
# 只报数不判级（方差过大/风格依赖，FAIL 级容易误伤正常文本）
...
def check_abuse(text, profile='body', active=None, doc_cfg=None):
    ...
    # 顺滑句对占比（语义平滑度代理：相邻句二字组Jaccard≥0.25的句对占比；值越高越顺滑）
    if len(sent_texts) >= 6:
        smooth_pairs = 0
        total_pairs = 0
    ...
    # v3.3 分割线残留：整行 =/-/*/_ 连续3+（代码/Markdown习惯）
