...
> pop-comic-content 出片动效用「JS 驱动逐帧渲染」：单页 HTML 暴露 `window.render(t)`，由 agent 逐帧设定时间并渲染。**v0.8 起画面主体是漫画分格裁剪**（按 `分镜标注.json` 裁剪格子，横向胶片与相机平移），叠加字幕淡入，**不使用 scale 缩放**（漫画是分格叙事，缩放会破坏格子与阅读节奏）。具体逐格阅读相机写法见 `references/scene-template.md`，本文件讲解通用辅助函数、时间轴与命令。

## 与 pop-video-brand 渲染的差异

| 维度 | pop-video-brand（品宣） | pop-comic-content（漫画） |
|:--|:--|:--|
| 画面主体 | HTML 元素搭建（文字与 UI 截图） | **漫画分格裁剪（panel）** |
| 画布 | 1920×1080 横版 | **1080×1920 竖版** |
| 动效 | 元素位移与缩放 | **格间平移滚动与字幕淡入** |
...
**逐格阅读相机**：见 `references/scene-template.md`，使用横向胶片 strip 与 `translateX` 做格间平移过渡（ease 缓动 0.45s），**不使用 scale**。字幕按口播时间窗淡入淡出，**每条字幕必须带出场淡出**（`app(进场)*out(出场)`）；本页第二条字幕进场前先淡出第一条，否则两条会叠加重叠。
...
> 字幕生命周期公式：`app(进场起,进场止) * out(淡出起,淡出止)`。进场止至淡出起之间为字幕驻留期（对齐配音段时长），淡出起设在下一条进场前 0.3s。**v0.8 起字幕按时间窗显示**（`t>=start && t<=end`），不得用格子 id 匹配字幕 id（两者本就不同）。

## 时间轴设计（每页时长 = 该页口播总时长 + 0.5s 余量）

- 从 `时长清单.json` 读每句口播 `duration_sec`，按页累加得到每页起止时间。
- 每页为一个声景，页面切换用 `out` 淡出、下一页 `app` 淡入（0.3s 交叉）。

## 常见坑

- 分格裁剪用 `object-fit:cover` 填满，不得改变布局尺寸；相机在 strip 的 `translateX` 上实现，**禁用 scale**。
...
- 中文字体必须显式设置（`Noto Sans CJK SC` 或 `WenQuanYi Micro Hei`）。
- **字幕按时间窗显示，不得用格子 id 匹配字幕 id**（v0.8 修过此坑，匹配永远不中，导致字幕永不显示）。
...
> 录屏方案通过注入自播时钟驱动已有的 `window.render(t)`，不改变动画逻辑，画面与逐帧方案一致。`--preset veryfast` 最快，`fast`、`medium` 质量更好，`--crf 18` 保画质。实测 57s 竖版视频全程约 86s 出片（逐帧方案仅渲染就需 7 至 10 分钟）。