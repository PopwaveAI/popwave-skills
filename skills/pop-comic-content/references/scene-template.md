...
> pop-comic-content 出片专用。**v0.8 起废弃 Ken Burns 缩放模板，改采用「逐格阅读」**：按 `分镜标注.json` 把分享成品图按格裁剪，相机在格与格之间做**纯平移（translate）滚动**，一格一格把漫画"读"过去。**不得放大或缩小（scale）**，因漫画是分格叙事，缩放会破坏格子与阅读节奏（插画才适合推镜）。

## 为什么改掉缩放模板

老板实测 v0.7 的缩放模板（T1~T5 Ken Burns）后予以否决：漫画是分格阅读顺序，缩放推拉既不尊重格子边界，又制造"剪映拼接"式的生硬跳切。逐格阅读用**切格与平移过渡**还原漫画本身的阅读节奏，画面连续，无生硬间隙。

## 核心结构

1. **分镜标注**：`分镜标注.json` 定义每页的格子 `bbox`（归一化 0~1，`[x0,y0,x1,y1]`）。**bbox 底部不超过 0.88**，向内收缩 gutter，避开分享成品图底部的 popwave 水印条。
...
5. **字幕按时间窗**：与口播 seq 时间轴对齐，字幕在对应口播窗口内淡入淡出（**不依靠格子 id 匹配**，否则永不显示，v0.8 已修此坑）。
...
- `bbox`：格子归一化坐标，**底部不超过 0.88**（水印条之上），向内留 gutter，避免裁到相邻格。
...
> **字幕按 `SUBS[j].start/end` 时间窗显示**，不得用格子 id 匹配字幕 id（两者本就不同，匹配永远不中，字幕永不显示，v0.8 已修）。

## 时间轴设计

- 每页时长为该页 `seq` 口播时长之和加 0.5s 呼吸量；页内每格展示时长为页时长除以格数（视觉均匀）。
- 口播 seq 时间窗：从页首起，按各 seq 实际配音时长累加。
- 字幕窗口为口播窗口起止各增减 0.2s。

## 验收标准（出片前逐页核对）

- 每格取景完整，格子边界正确，无黑边，无水印残留（bbox 底部不超过 0.88）。
