...
> 本技能是画风三组测试。用固定三组模板（场景、角色、多格剧情）逐个画风验证 DNA 执行力，产出"画风能不能用"的通过判定。v1.4.0

## 职责范围

对 `pop-visual-style` 的画风 DNA 库做**三组内容形态测试**，验证画风 DNA 在不同内容下是否稳定执行：

- **T1 场景向**：环境为主，验证画风能否撑起全景与氛围
- **T2 角色立绘向**：角色为主，验证画风能否撑起角色与主观特征
- **T3 多格剧情向**：一页多格，验证画风能否撑起最终漫画产物

**核心价值**：一个画风仅"适合场景"不够，必须能在角色、分格剧情里也成立，才算真正可用。三组全过即画风稳定；部分过即画风有偏科，需标记偏科项。
...
### Step 2: 生成三组 config，执行 `scripts/build_3test.py`
...
- 输出"画风通过判定表"：T1、T2、T3 各过或不过，并标注偏科项
...
| # | 红线 | 违反后果 |
|:-:|:-----|:---------|
| ❌1 | **必须走固定脚本** — 不得现场手写提示词或临时修改三组模板，必须用 `build_3test.py` 生成 config | 变量不隔离，测试不可比 |
| ❌2 | **控制变量** — 三组模板的非画风部分（构图、光影、场景、角色）永久固定，不得掺入画风特征 | 无法判断是画风还是模板问题 |
| ❌3 | **三组全测** — 每个画风必须执行 T1、T2、T3 三组，不得只测一组 | 画风偏科被漏检 |
