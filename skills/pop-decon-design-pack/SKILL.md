...
description: "当用户说'拆书/提取设计包/ETL拆分/白描卡'时启用。双维度：输出格式 precision(v4设计包3层+1区)/fast(瘦身白描卡4段式) × 处理方式 quality(每章1子agent)/performance(每30章1子agent合并)。全部走子agent派发，无API脚本。任务开启前强制确认模式。产出供下游volume/setting消费。"
---
...
> execution.mode: precision/fast | execution.strategy: quality/performance | 全部走子 agent 派发，无 API 脚本 | 强保障：本 SKILL.md 由 host 层每次 run 强制注入 | 弱保障：steps/ + references/ 需 agent 主动 readFile
...
| 2 | 模式确认 + 派发子agent提取（双维度） | precision: `设计包v4/chXXX-设计包.md` / fast: `白描卡/chXXX.md` | 已确认模式 + 结构完整 | `steps/step-2-batch-process.md` |
...
6. 子agent落盘错误目录 — 子agent用相对路径产出了临时目录 → 退回用绝对路径重派
7. **未确认模式擅自执行** — 任务开启前未与用户确认 strategy+mode 就提取 → 退回重确认
...
| `steps/step-2-batch-process.md` | Step 2 派发提取时 | 模式确认 + 子agent派发流程 |
