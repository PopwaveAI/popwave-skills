...
| 输入 | 来源 | 输出 | 下游 |
|------|------|------|------|
| 设计包+故事机制DNA+名场面库+Wiki骨架(可选) | Phase 1/2 | 骨架层+血肉层+角色层+叙事资产层 | pop-decon-prd |

管线：Phase 1(设计包) → Phase 2(故事DNA+名场面+读者体验) → **Phase 3(设定)** → Phase 4(立项)
...
- Step 15 角色弧线 → `steps/step-15-character-arc.md`（从故事DNA有效事件清单提取，含状态/心态/触发事件）
...
2. **Phase 2 未完成就归纳** — 故事机制DNA产出缺失 → 退回 Phase 2
...
| steps/step-15-character-arc.md | 角色层执行时 | 角色弧线（从故事DNA提取） |
