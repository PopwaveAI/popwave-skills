...
**背景**：老板定调——每个 C 端专家需要一份面向作者本人（非产品经理/AI 专家）的首次对话引导语，用"你能从一本好书里拆出什么"的场景快速建立认知，而非介绍内部架构。

**改动**：

- 新增 `references/onboarding-guide.md`：C 端口吻引导语（一句话说清 + 你有这些玩法 + 你不用担心 + 就这样开始），覆盖剧情线/感情线/人物/力量体系/世界观/爽点/文风多维拆解
- `SKILL.md`：新增「🚪 首次对话引导」区块——用户首次触发时直接输出引导语全文，再进 Step 0 下载
- 版本至 v22.1.0
...
- **管线地图更新**：Phase 2从"L2单元卡+卷纲"改为"故事机制DNA+名场面库+读者体验曲线+卷纲"；Phase 3从"设定层+角色层+势力层+叙事资产层"改为"设定+角色（含角色弧线）+伏笔追踪"
- **路由建议更新**：step-1-pipeline.md的Phase 2-4路由建议对齐新产出物
- **质量门禁更新**：Phase 2门禁从"L2单元卡≥3条"改为"故事机制DNA≥1份+名场面库≥1份+读者体验曲线≥1份+卷纲≥1份"；Phase 3门禁从"L1六件套"改为"骨架层+血肉层+角色层+叙事资产层"
- **沉淀路径更新**：从"剧情库/{标签}/"改为"设计/"
- skill.json版本同步至21.0.0
