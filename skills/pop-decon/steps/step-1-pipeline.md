...
2. Phase 2 — pop-decon-volume（≥100章时推荐）
   从叙事白描提取：故事机制DNA + 名场面库 + 读者体验曲线 + 卷纲
   产出：设计/故事机制DNA-卷N.md + 设计/名场面库.md + 设计/读者体验曲线-卷N.md + 设计/卷纲/

3. Phase 3 — pop-decon-setting
   归纳骨架层(力量体系) + 血肉层 + 角色层(含角色弧线) + 叙事资产层(伏笔追踪)
   产出：设计/各设定文件

4. Phase 4 — pop-decon-prd
   消费Phase 1-3全产出，产出全书立项设计
   产出：全书立项设计.md

5. 沉淀确认
   逐模块确认产出已沉淀到项目本地文件夹（不入库pop-trope-library）
```
...
| Phase 2 | 故事机制DNA≥1份 + 名场面库≥1份 + 读者体验曲线≥1份 + 卷纲≥1份 | 退回 Phase 1 |
| Phase 3 | 骨架层(力量体系) + 血肉层 + 角色层 + 叙事资产层齐全 | 退回 Phase 2 |
...
全管线 Phase 1→4 完成后，逐模块确认产出已沉淀到项目本地文件夹并更新索引。**不入库pop-trope-library**（library库已优化掉，decon拆解产出直接沉淀到项目本地）。

| Phase | 沉淀位置 | 通过标准 |
|:------|:---------|:---------|
| Phase 2 | `项目本地/设计/` | 故事机制DNA≥1份 + 名场面库≥1份 + 读者体验曲线≥1份 + 卷纲≥1份 |
| Phase 3 | `项目本地/设计/` | 骨架层+血肉层+角色层+叙事资产层齐全 |
| Phase 4 | `项目本地/` | 全书立项设计.md存在 |
| 文风DNA | `项目本地/素材/文风锚定.md` | 文风档案存在 |

> 沉淀路径默认为当前创作项目根目录下的对应子目录。如用户未指定项目，询问沉淀到哪个项目。

完成后告知用户："拆书完成，产出已沉淀到项目本地文件夹。写作管线可直接从项目本地消费。"
...
> 全管线 Phase 1→4 + 沉淀确认全部完成后，输出摘要告知用户各模块沉淀状态。
> 询问是否需要转换为写作项目或仅保留为分析。

---
## ⛔ 加载门禁 + 下一步指引
> 下一 step：下游 skill pop-decon-design-pack（Phase 1）
> 什么时候进入下一步：源文件确认 + 量级确认 + 路由建议已输出
