...
### 剧情-情绪打分表结构重构承接（老板拍板）：双8分核验改情绪达标核验

- 先反核章纲标注（峰值标注是否符合打分表情绪定义+铺垫成本，6 分以上必须铺垫引爆、无跨章铺垫标 8=虚标，虚标直接打回 outline 不核正文），再核正文兑现（每章 ≥2 个峰值点，普通章 5-6 为主，8 分=见血/见死/见不可逆或同级正向震撼，假致命不算）。
- 峰值示例对照改新表（愤怒/害怕·8分=在乎之人面临不可逆威胁·临界不越界，喜悦·8分=跨章铺垫的长线目标兑现）。
- 阶段性回看情绪点兑现改峰值达成率+情绪单调性。
