...
# 1. 涌现资产（优先级最高：设定库/人物库/剧情线）
$assetDir = Join-Path $projectDir "涌现"
$assetFiles = @('设定库.md', '人物库.md', '剧情线.md')
...
- 如果总字符数超过 10 万，只加载：设定库/人物库/剧情线全文 + seed/燃料文件全文 + 最近 3 章正文全文 + 最近涌现日志全文 + 压缩归档摘要。
...
这不是文风DNA，不做修辞模仿。它用于防止新文本漂成行业评论、严肃文学、普通科技新闻或另一本书。
