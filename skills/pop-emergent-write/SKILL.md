...
**文风DNA全文读取红线**：
- 文风锚定文件（`涌现/文风锚定.md`）必须用 exec（Get-Content -Raw）读取全文，禁止用 Read 工具读取——Read 工具对大文件有随机截断（实测保留率 9%-80%），会丢失场景卡原文摘录（v4 模板核心价值）。
- 全文加载协议已使用 Get-Content -Raw，符合此要求；但如果跳过全文加载协议单独读取文风锚定，必须用 exec。
- 禁止只读取文风锚定的目录、摘要或概述——必须读取包含所有场景卡原文摘录的完整文件。
...
4. **文风锚定全文必须保留在上下文中**——字段填写只是用于注意力聚焦，不得用字段替代全文。写正文时 agent 必须能随时回看其他场景卡的原文摘录。

**笔触 vs 内容红线**：
- 文风DNA传递的是**笔触**（句式怎么断、感官怎么排、叙事距离怎么站、段落怎么呼吸、对话怎么引导），不是**内容**（打谁、怎么打、打完想什么、战术怎么设计、角色怎么复盘）。
- 文风DNA的原文摘录是让 agent **感受作者怎么写**，不是让 agent **抄作者写了什么**。
- 禁止从文风DNA原文摘录里提取剧情设计、战斗表现、角色思维作为创作要求。原文里索伦的战术计算、击杀余韵、boss战濒死模板是深渊主宰的**剧情内容**，不是文风。
...
- **禁止摘要/压缩文风锚定后传递**：文风锚定全文必须存在于写正文 agent 的上下文中，不得压缩成抽象要点。如果调度子 agent 写正文，文风锚定全文必须作为 task 内容注入，禁止用"文风DNA要点/关键特征/继承特征"等摘要形式替代全文。
