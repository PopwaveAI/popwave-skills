...
本步骤消费 write 的 4 类必读输入，抽取本章写作包。引用 PRD §4.1（骨架）、§4.2（owner/read_policy）、§4.5（execution.mode）。
...
- 不自行翻库补写。
- 停止并要求先执行 pop-emergent-review / ledger 更新 current-state。
...
- formal：current-state + soul + 最近正文 + 用户要求 齐全且达标。
- draft：必读输入有缺口但用户要求继续。
- trial：快速试做，不落盘。

缺 current-state 或 soul 不得标 formal。
