...
### 日常路由上移专家提示词；状态源统一为 状态.md

> **根因**：番茄专家收敛调研确认——pipeline 应退回「一次性安装器」，常驻路由循环（每次对话读状态→路由→更新）属每轮行为契约，应收进专家提示词（阶段地图+灵活调度）；跨族状态源统一为 状态.md（机器），html 仅供展示、agent 不读。

**改动**：
- **删除「路由循环（每次对话开始时）」** 常驻 loop，改为「日常路由」参照节——pipeline 只管一次性安装/导入，日常写正文/审核不经过本 skill
- **状态源统一**：project-state.md → `状态.md`（唯一机器状态源）；project-state.html → `总控.html`（展示面板，按需导出，agent 不读）
- `pop-fanqie-seed` 状态读取同步改为 `状态.md`
...
### html展示面板降级为可选按需，project-state.md确认为唯一机器状态源

> **根因**：番茄专家收敛调研确认——html是供人看的展示面板，无任何自动化环节消费它，每轮更新state强制跑脚本渲染属过度设计；且提示词误称读「项目总控.html」，实应读机器状态源 project-state.md 的状态片段。

**改动**：
- **定位拆分**：project-state.md = 唯一机器状态源（mode/phase/current_chapter/底牌就绪）；project-state.html = 人看展示面板，可选按需导出，不参与任何路由/决策
- **去强制化**：路由循环/红线#2/0e/1d/质量门/速查表/state更新方法 中「每次更新后必须同步生成html」改为「仅用户要看时按需运行 generate-state-html.py」；html移出质量门
- **读法明确**：路由循环第1步明确「提取状态片段（phase/current_chapter/底牌就绪），只读机器状态源，不读html展示面板」
- **与专家提示词对齐**：配套番茄专家提示词第1条改为「先读状态片段」，不舔html全量
