...
**改动**：
- **steps/ 目录删除**：step1.md / step2.md 两件全合入 SKILL.md 对应节（Step 1-3：正向符合性1a-1i+番茄底线+正文质量AI味15项/高疲劳词3级/结构性4项+DNA 8项+好看度4问+反向充足性4维度；Step 4：双文件沉淀模板+门禁+回复格式）
- **执行模式明确**：Step 1-3纯只读审计（读正文+读设定核对）天然适配子agent派发（审计官）——子agent执行审计回报结论，落盘（Step 4双文件更新）归主agent；无子agent环境主agent直执四步
- **内容精炼**：AI味15项patterns/高疲劳词3级/结构性4项/DNA 8项/番茄底线等审计表全保留；三步门禁checklist与落盘后checklist压为要点行并入SOP；红线删除读取协议（steps加载链已不存在）8→7条；速查表删除（steps条目已内联）
- skill.json version 4.11.0→5.0.0
