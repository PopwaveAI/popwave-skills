 ...
 *
 * @param {string} base64Image  - base64 编码的图片数据（不含 data:... 前缀）
 * @param {string} mimeType     - 图片 MIME 类型，如 'image/jpeg' / 'image/png'
 * @param {string} [customPrompt] - 可选，覆盖默认 QA_PROMPT
 * @param {object} [opts]         - 可选，{ timeout, model }
 ...
 * 按顺序调用，避免并发触发限流
 *
 * @param {Array<{ htmlPath: string, skeletonId: string, base64: string, mimeType: string }>} images
 * @returns {Promise<Array<{ htmlPath, skeletonId, text, score, usage }>>}
 */
