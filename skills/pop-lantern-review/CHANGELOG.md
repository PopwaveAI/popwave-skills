...
pop-lantern-review v1.0.0：重建自 pop-snow-review v1.8.0 全量 fork，待减法改造。
...
> **背景（老板批点）**：黄金三章作为独立阶段与后续流程割裂，三系列统一移除；snow 系列每章爽点把关强化——outline 章纲已加爽点标注字段，review 承接核验闭环。

- **满意判别标准新增第 7 条爽点密度**：对照章纲爽点标注逐条核——每章 ≥1 个重大爽点（无爽点章不连续），密度目标 ≥2；爽点须含**发生（事件本身）＋反应（读者情绪落点）＋影响（对状态/剧情的实际改变）**，只写事件不写反应与影响=假爽点；章纲标注的爽点正文没兑现=打回。
- **爽点类型 9 类**（章纲选1-2）：战斗胜利/夺宝获资/成长升级/打脸装逼/剧情推进/关系推进/身份跃迁/危机拯救/震撼场面。
- **第 2 步章日志明确第 5 项爽点·钩子**（模板已有字段，改为必填项）。
- **速查表**：chapter-card 模板说明补"含爽点·钩子字段"。
- 同步三件套：SKILL.md / skill.json（1.9.0）/ CHANGELOG。

## v1.8.0 | 2026-09-10

### 吸收 Mirror 03 群像管理：review 第 3 步撤角色时加"群像卫生"检查（活跃≤5-7/功能合并/定期清理）

> **背景**：阶段6评估的结论——Snow 的 review 已有"退出档案＋全书日志只记活跃"（退场交带有留痕），outline 舞台背景已有"在场人物每人一句今天要干嘛＋承接8项核对角色状态"（每个活跃人物有当场理由），但缺 Mirror 03 群像管理的**名单卫生守卫**：同时活跃人数上限、功能重叠合并、每 ~20 章清理活跃角色表。这三条 Snow 没有，并进去能防止角色越写越多记不住。

- **第 3 步「群像卫生（撤角色即检查）」**：把人物撤进退出档案时，对照当前"活跃"角色表顺一遍——①同时活跃 ≤5-7 人（超出读者记不住）；②功能重叠的合并一个（两个智囊不如一个智囊+一个执行者）；③每个活跃人物都要有"本章在场理由"，没理由的不进画面；④退场必须交代（死亡/离开/转背景板），不许人间蒸发；⑤每 ~20 章顺手清一次活跃表（谁还起作用、谁该退）。
- **吸收纪律**：镜像 Mirror 03 群像管理的 5-7 上限/功能合并/定期清理，与 Snow 既有"退出档案/在场靠舞台背景"互补不重复；审查存量清单（Mirror 09）判定已被 review 三把尺＋承接8项核对覆盖，不并入。
- **不动** word-count.ps1 / deai_gate.py / outline 已有在场人物逻辑。
- **1+1>2**：review 从"撤一个记一个"升成"撤时顺带管名单规模"，群像不失控不需要单独群像 skill，靠既有退出档案闸口顺带守住。
- 同步三件套：SKILL.md / skill.json（1.8.0）/ CHANGELOG。

## v1.7.0 | 2026-09-08

### 资料门收敛：review 不设资料门核验（删红线第5条，只做产后账本维护）

> **背景**：老板把资料门收敛到 seed/stage/plot/outline 四环节，review 只做「定稿→章日志→全书日志」产后维护，不拦截资料问题。

- **删除红线第5条「资料门核验（research 兜底闸）」**：缺料硬写由四环节资料门＋write→outline 打回链路兜底，review 不再补一道拦截闸，保持职责单一。
- 同步三件套：SKILL.md / skill.json（1.7.0）/ CHANGELOG。

## v1.5.1 | 2026-09-08

### 消毒核验闭环闸（配合六 skill 消毒挪进 SOP）

> 背景：西幻DND 实跑发现消毒停留在红线提醒层、未被实际触发（老板指令：把消毒逻辑挪进 SOP 落盘步骤，并加核验）。核验放在 review——归档闸门核验上游"消毒已跑"。

- **第 1 步升级"确认满意＋消毒核验"**：核验上游消毒回执（write 正文 body 闸 FAIL 清零＋outline 章纲 `消毒:ch{NNN}-章纲|…` 回执）；章纲缺回执→review 补跑 deai_gate doc 报告闸一次并注明补消毒；正文回执缺失→本章日志备注、提示 write 下章前补跑。报告闸只报不打回，核验不阻塞归档、必留痕。
- **第 2/3 步各接"落盘后半步=消毒"**：章日志、全书日志（含本轮退出档案追加）落盘后自跑 deai_gate doc 报告闸，回执一行。
- 红线原第5条"落盘后消毒"删除——消毒与核验均入步骤，红线不重复约束。
- skill.json description 同步消毒核验职责。

## v1.5.0 | 2026-09-07

### 落盘后消毒闸接线（doc profile）

- 红线新增第5条：章日志/全书日志落盘后自跑 `python skills/pop-lantern-pipeline/scripts/deai_gate.py <文件> --profile doc --json`——报告闸只报不打回：档案措辞贴事实，黑话/套路句式按 WARN 定位自查；工程标签（字数栏/线名）不在消毒范围，零风险机械项可 `--fix`。
...
- **脚本迁出**：`scripts/word-count.ps1` 迁回 write（`skills/pop-lantern-write/scripts/`），review 侧 scripts 目录删除，杜绝双源。
...
### 更名：pop-review → pop-lantern-review
