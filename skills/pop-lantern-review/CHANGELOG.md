...
### 好奇模块新增承接（老板拍板）：钩子核验对照打分表好奇模块查档

- 「钩子承接」「剧末钩子立住」两检查加好奇·X档核验：章末留新缺口/剧末钩子对照打分表好奇模块查档，伪钩子（信息不足没法猜）=打回。
- 引用方只查表不复制（引用纪律生效）。

## v3.5.0 | 2026-09-11

### 情绪引用去耦（老板拍板）：8分定义/临界规则/反例一律打开打分表查级

- 打分表内容不再抄进本文件，本文件只留核验动作（打开打分表第四节细则+第六节反例区核）。
- 打分表改动引用方不跟着改。

## v3.4.0 | 2026-09-11

### 剧情-情绪打分表结构重构承接（老板拍板）：情绪达标按情绪分维度核

- 先反核章纲标注：8 分事件须符合打分表情绪定义+铺垫成本（6 分以上必须铺垫引爆，无跨章铺垫标 8=虚标），虚标直接打回 outline 重排，不核正文。
- 正文兑现判据改"目标 {情绪}·{分值} 实际只到 {情绪}·{低分}，差 2 级以上打回"；双8 兑现改峰值兑现（章纲标注的 8 分事件正文没兑现/没写透=打回）。

## v3.3.0 | 2026-09-11

### 情绪达标补「章纲标注反核」：先反核章纲 8 分标注（虚标直接打回 outline），再核正文兑现（复盘第一个点：剧情情绪表没发挥作用）

> **背景（老板复盘第一个点）**：ch001 章纲把"断桅砸死水手（配角死）""认出记号（信息钩子）"标成双 8 峰值——照剧情-情绪映射表核都是虚标：线1·8 要的是"读者在乎之人面临不可逆威胁"，线5·8 要的是"命运改变的收获"；且映射表线1·8 原示例"主角本人命悬一线"本身也站不住——读者默认主角不死（主角光环稀释），调动不了情绪（老板：主角掉海里也没情绪，在乎之人尤其主角团女性面临威胁/性侵风险才调动愤怒，但绝不能真发生=重大毒点）。两道都落：outline 自检闸＋review 反核（本包落反核）。

- **满意判别标准 #8 情绪达标改两步**：**先反核章纲标注**——章纲 8 分事件是否符合剧情-情绪映射表对应线定义？（线1威胁生死·8分＝**读者在乎之人**（主角团/至亲/爱人，尤其女性角色）面临不可逆威胁·临界不越界；"主角本人命悬一线/配角死亡充数/信息钩子"=虚标）→ **虚标直接打回 outline 重排，不核正文**；**再核正文兑现**——实际强度 vs 目标值差 2 级以上打回、双8 未兑现打回（判据不变）。
- **剧末回看·情绪达标回看同步加反核**：先反核单元编排的情绪目标标注（8 分事件须为"在乎之人面临不可逆威胁·临界不越界"，虚标=编排层问题标给 plot 下单元修正）。
- **配套**：剧情-情绪映射表修订（plot 包内容库）——线1·8 示例改"主角团女性被掳为人质、衣领撕开、烙铁贴脸、不可逆伤害即将发生（临界不越界）"；新增主角光环稀释说明/主角光环假威胁反例/配角死亡充数反例/临界状态管理规则；读者心跳库（涌现流）同源同步。outline v3.4.0 同步加 8 分落级自检两步闸。
