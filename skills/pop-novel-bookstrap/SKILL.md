...
Step 3 ─ 设定与验证（Phase 1 → 1.2 → 1.3 → 1.5 → 3 → 4 → 5）
  ...
  ★ 输出 Phase 3: project.yaml + 状态/角色/角色卡
    ★ 主角卡包含 core_desire(external_goal+internal_need 两层) + 末尾成长轨迹22行逐章段
...
Step 3 ─ 大纲还原 + 确认 + 交接（Phase r4 → r5 → r6）
  volume-XX+act-XX 还原 → 卷大纲确认 → 交接验证报告
```
...
| Phase r4 — 卷纲/幕还原 | `phases/phase-r4.pe.md` | — |
