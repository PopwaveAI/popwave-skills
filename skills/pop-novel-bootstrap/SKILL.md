...
description: 从故事引擎到可执行项目骨架的开书管线。当用户说"开书"/"新书"/"启动项目"/"设世界观"/"续写"时启用。支持 forward（新书）和 reverse（续写）两种模式。
---

# 开书启动

从故事引擎到可执行项目骨架的完整开书管线。产出 story-engine.yaml、L1 设定层、数值体系、constitution.yaml。
...
| ❌6 | **constitution.yaml + project.yaml 路径注册** — 产出文件路径已注册 | [ ] |
