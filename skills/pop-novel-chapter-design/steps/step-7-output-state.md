...
按 `templates/fact-skeleton.md` 完整格式，写入：
- 基础信息（幕/章号/标题/核心目的/场景类型/爽点等级/字数）
- 事件链（3-5 个事件，每个含地点/角色/内容/情绪/信息释放/估计字数）
- 情绪节拍（起点→发展→高潮→终点）
- 信息释放落地清单
- 钩子
- 与上章衔接
...
按 `templates/character-card.md` 完整格式，写入：
...
根据本章所有角色的 after 状态，更新 entity-snapshot：
- 角色状态（等级/位置/装备/心理）
- 事件日志（追加本章事件摘要）
- flags（新增/更新/移除）
- timeline
...
- [ ] entity-snapshot 已正确更新（after → before for 下一章）
- [ ] constitution 红线全部通过
- [ ] 骨架文件有 `_meta.version: "1.0"` 元数据
- [ ] 人物卡文件有 `_meta.version: "1.0"` 元数据
