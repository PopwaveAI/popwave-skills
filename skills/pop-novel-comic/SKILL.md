...
- 读小说正文 ch{NNN}.txt → **话数规划**（信息负载评估：关键信息数+场景数+角色数+故事线数 → 轻/中/重分级 → 1话/拆分多话决策；拆分时每话独立走完整流程，每话有完整弧线+钩子衔接）
...
- 产出 **导演卡.md**（话数规划+章节概要+叙事主线+改编灵魂+角色变化+关键信息审计表+改编策略表+转化帧设计+旁白文案）
- **🚪 门禁：导演卡确认**（话数规划合不合理 / 叙事主线清不清楚 / 改编灵魂对不对 / 关键信息有没有漏+分级对不对 / 改编策略对不对 / 转化方案行不行）
...
v2.9.1 | 2026-07-31 | 话数规划机制。step1-director 新增 §2.5 话数规划：改编分析前先评估信息负载（关键信息数+场景数+角色数+故事线数四项指标），按轻/中/重分级决定 1 话/拆分多话。"重"负载（任意两项指标达标：≥9信息/≥6场景/≥7角色/≥2故事线）必须拆分，每话独立走完整流程，每话有完整弧线（起因→发展→决策→后果）+ 钩子衔接。导演卡模板/门禁/自检同步升级。根因：玄鉴仙族第1话将2章原文（9信息+7场景+7角色+2故事线）塞进12格，信息超载2-3倍导致读者看不懂——不是叙事结构问题而是篇幅不够。
