...
v1.0.0 | 2026-07-29 | 初始版本：基于 R15 测试验证的 5 阶段管线（章节解构→角色定妆→逐格分镜→HTML组装）。2 步结构：Step 1 章节解构+分镜脚本+门禁A，Step 2 角色定妆+分镜生成+HTML组装。复用 pop-novel-visual 的 API 脚本和提示词指南。核心管线：Agent 做编剧（拆分镜），Seedream 做画师（生成画面），HTML 做排版（组装漫画页）。
