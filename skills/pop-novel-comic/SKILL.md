...
> 网文漫画连载管线。DeepSeek 做编剧和项目管理（导演卡、拆分镜、管角色库、记状态），Seedream 做画师（高并发生成画面），HTML 做排版（Pillow 备选），Playwright 截长图。v3.2.0
...
- **漫画化改编分析（三层推导）**：
  ...
  - 格层：**分格设计**（信息级别→格子大小，页面节奏→格子排列，布局选择决策树→分格设计表，**构图手法选择→每格构图骨架**）
...
- 读导演卡 → **继承分格设计表**的布局类/节奏/效果类/构图手法，补充画面提示词/定妆图版本/场景主镜/对白文字 → 形成分镜脚本表
