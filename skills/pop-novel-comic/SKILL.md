...
> 网文漫画连载管线。DeepSeek 做编剧和项目管理（导演卡、拆分镜、管角色库、记状态），Seedream 做画师（生成画面），HTML 做排版。v2.8.0
...
- **漫画化改编分析**：改编灵魂三问（灵魂冲突/避开俗套/求新钩子）→ 内容分段标注 → 关键信息审计（含证据链：来源段落+原文摘录）→ 逐条定改编策略 → 转化方案设计 → 信息覆盖率自检
- 产出 **导演卡.md**（章节概要+改编灵魂+角色变化+关键信息审计表+改编策略表+转化帧设计+旁白文案）
- **🚪 门禁：导演卡确认**（改编灵魂对不对 / 关键信息有没有漏+有无原文来源 / 改编策略对不对 / 转化方案行不行）
