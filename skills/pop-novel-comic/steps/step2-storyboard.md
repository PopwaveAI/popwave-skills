...
> **v2.10.0 变更**：分格设计表已在 Step 1 确定了每格的布局类、节奏、效果类和构图手法。Step 2 在此基础上**补充执行细节**（画面提示词、定妆图版本、场景主镜、对白文字），形成完整的分镜脚本表。不重新决定布局类、节奏和构图手法。
...
每帧必须标注出场角色、定妆图版本、场景组、布局类、节奏和构图手法：

| 帧号 | 所属页 | 场景组 | 场景 | 角色动作 | 机位 | 构图手法 | 情绪 | 节奏 | 布局类 | 效果类 | 出场角色 | 定妆图版本 | 场景主镜 | 对白/文字 | 画面提示词 |
|:-----|:-------|:-------|:-----|:---------|:-----|:---------|:-----|:-----|:-------|:-------|:---------|:-----------|:---------|:---------|:-----------|
| 1 | P1 | A | 破屋，雨夜 | 薇薇安从米袋抓米 | 全景 | 三分法 | 压抑 | 慢 | panel-full | — | 薇薇安 | char-vivian-v1 | — | 旁白:没有食物了 | [提示词] |
| 2 | P1 | A | 破屋灶台 | 薇薇安抱老狗 | 中近景 | 框架式 | 温暖 | 慢 | panel-half | — | 薇薇安 | char-vivian-v1 | frame1 | 对白:又不会偷东西 | [提示词] |
| 3 | P1 | A | 门外 | 脚步声逼近 | 特写 | 倾斜构图 | 恐惧 | 快 | panel-half | — | 无 | — | frame1 | 旁白:脚步声 | [提示词] |
| 5 | P2 | B | 集市 | 薇薇安穿新衣出行 | 中景 | 三分法 | 明亮 | 慢 | panel-full | — | 薇薇安 | char-vivian-v2 | — | 旁白:第一次穿上干净的衣裳 | [提示词] |
| 8 | P3 | C | 门口 | 索伦睁眼 | 特写 | 大留白 | 震惊 | 慢 | panel-hook | — | 索伦 | char-soren-v2 | — | — | [提示词] |
...
```
[Seedream 执行串(画风,含参考作品)]。[构图骨架串(机位+构图手法)]。参考图中的人物形象，[视觉锚点串]。[微表情串（如有情绪）]。[角色动作描述]。[场景环境描述(精简≤2句)]。[情绪氛围]。[风格保真约束]。
```

**关键规则**：
- **首句必须是 Seedream 执行串**——从漫画角色库的「Seedream 执行串」字段复制，放在提示词最前面（高权重位）
- **第二句必须是构图骨架串**——从导演卡分格设计表的「画面方向」+「构图手法」两列组合翻译为英文，放在画风串之后（高权重位）。格式：`{机位英文}, {构图手法英文}.`。例如：`EXTREME LOW ANGLE, looking up. Diagonal composition with character at lower third intersection.`。**缺失构图骨架串=Seedream默认"主体居中+平视"=中庸构图**
