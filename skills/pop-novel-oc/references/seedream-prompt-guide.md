...
> 基于实测验证的工程图纸级写法。适用于商业级封面、角色立绘加强版、漫画关键帧等需要最高质量输出的场景。比 V3 结构化公式更严谨，新增镜头语言、渲染要求、硬约束三个维度。
...
4. 取 `constraint` 字段作为风格保真约束（加入高精度4块结构的HARD CONSTRAINTS）
...
| 传统仙侠/古言 | 国漫玄幻厚涂、国风水墨仙侠 | 水墨用LT3平光 |
| 言情/甜宠 | 少女水彩言情、日系赛璐珞、轻小说插画风 | LT2柔光通透 |
| 悬疑/诡异 | 暗黑悬疑高对比、伊藤潤二、黑执事 | LT1减法照明 |
| 热血/战斗 | 日系热血战斗、韩漫暗黑厚涂、赛博边缘行者 | LT1或LT2 |

> 完整36种画风详见 `skills/pop-novel-image/references/style-dna-library.json`。立绘场景画风前置（放4块结构前段），图生图时按参考配合模式处理。
