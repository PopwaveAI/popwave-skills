...
> 完整定义 + 案例 + 设计方法 → `templates/payoff-guide.md`（全链路唯一权威定义）。
> 此表仅列出 plot 层面需要关注的密度约束和大/终极位置规划。

| 级别 | 密度约束 | plot 负责什么 |
|:-----|:--------|:-------------|
| 微爽点 | 每章 ≥ 5 个 | **不负责** — 归 prose-render 在写作中创造 |
| 中爽点 | 每章 ≥ 1 个 | 声明密度，不指定具体事件（归 chapter-design 逐事件标记） |
| 大爽点 | 间隔 ≤ 5 章 | **位置规划** — 在 Canvas 矩阵中标记章节号 + 前 2-3 章蓄力设计 |
| 终极爽点 | 每幕 ≥ 2 个 | **位置规划** — 幕中爆发 + 幕末收束，填入 payoff_distribution.positions |
