...
### 爽点频率红线（★ NEW v4.3）
> 平台留存率的第一性：读者每 2 章必须同时得到战斗爽 + 成长爽。两者不是「或」的关系——缺任一项该章段就不合格。

- [ ] **战斗频率**：连续 2 个 ☐ 的 M2 行 → 退回。每 2 章至少 1 次 payoff 类型含 combat 的章节。
- [ ] **成长频率**：连续 2 个 ☐ 的 M3 行 → 退回。每 2 章至少 1 次面板更新/等级提升/新能力解锁。
- [ ] **& 约束**：以上两个条件必须同时满足。仅通过战斗频率或仅通过成长频率 → 退回。每 2 章内既要打一次、也要升一次。
- [ ] **高潮章深度**：高潮章（蛇魔战/BOSS战/卷末终极爽点）的 combat 字段必须含：`enemy_level`（敌人段位）、子节拍展开（≥4 beats）、equipment_reward（具体装备掉落）。缺任一项 → 退回 Step 9。

### 上游角色卡消费（★ NEW v4.3）
> Act-01-人物.md 中主角和核心配角的数据必须从 bookstrap 产出的角色卡提取，不在 plot 中裸写。

- [ ] `act-XX-人物.md` 中主角的等级/属性/成长路线与 `L3-角色层/{主角}-主角卡.md` 一致
- [ ] `act-XX-人物.md` 中介于每条 S 线的核心配角的"动机/目标/关系冲突"被标注在对应情节线草案的 S 线定义中
- [ ] 配角卡缺失 → 退回 bookstrap Phase 3，不裸写角色
