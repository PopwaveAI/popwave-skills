...
description: 正文渲染/上色表达。消费 Design 的设计包产物（事实骨架+登场人物卡合并），直接应用 styles/ 文风DNA 渲染为可读正文。不判断剧情逻辑、不验证设定一致性。用户说"写正文/渲染这章"时启用。由 expert-writer 通过子 agent 调度。
pipeline:
...
| 步骤 | 详细指令 | 说明 |
|:-----|:---------|:------|
| Step 1 — 读入输入 | `steps/step-1-read-input.md` | 读设计包+styles+宪法+锚定章感知 |
