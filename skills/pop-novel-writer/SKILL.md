...
> 此规则优先级高于所有其他规则。
> 违反此规则 = 系统级违规。
> 每次写正文前（Phase 1 Director 之前）执行，不可跳过。

### 执行规则

写正文前，必须按以下清单读取文件。每读完一个标记确认。

```
□ 项目当前状态（上一章结尾 + global-summary.md + chapter-state.yaml）
  → 确定本章衔接点

□ 当前幕纲（02-大纲/卷XX/act-XX.yaml 或 act-{幕号}.yaml）
  → 确认本章预设的情绪目标、爽点类型、章末钩子

□ 书中宪法（02-大纲/constitution.yaml）
  → 确认本章不违反创作原则

□ 本期读者画像（00-总控/project.yaml → reader_profile 字段）
  → 确认本章写给谁看

□ L1 设定层（00-原始设定/L1-元设定层/01-06.md）
  → 确认力量体系、金手指约束、势力关系、物品作用范围

□ 数值体系（00-总控/数值体系/combat_capability.yaml + act_rank_schedule.yaml）
  → 确认当前阶段能力边界，防止跨级战力失控

□ ☆ 地理与时间轴（如存在：00-原始设定/地理-世界地图.md + 时间轴-编年史.md）
  → 确认场景地理关系和当前时间点
```

### 注意力校验原则

读文件后，agent 必须在 thinking 中显式确认：
...
如果无差异 → 继续。如有差异 → 以文件为准，修正认知后再产正文。

> **注意**：agent 倾向于"我记得"而非"我确认过"。读前的思考必须执行。
> 完成读前确认后，再进入 Phase 1 Director 设计。
...
| `tunshi` | `styles/tunshi.md` | 吞噬星空·硬核科幻升级流：精确参数+日常渗入世界观 |
| `zhetian` | `styles/zhetian.md` | 遮天·古典仙侠宏大叙事：文学化表达+意象堆叠+悬念 |
| `abyss` | `styles/abyss.md` | 深渊主宰·西幻黑暗写实：五感堆叠+情感浓度+残酷现实 |
...
├── style-dna-template.md   # 文风DNA全量模板（4层·30+维度）
 ...
 ├── tomato.md               # 番茄快节奏（日更平台优化）
 ├── tunshi.md               # 吞噬星空·硬核科幻升级流
 ├── zhetian.md              # 遮天·古典仙侠宏大叙事
 └── abyss.md                # 深渊主宰·西幻黑暗写实
```
