...
description: 正文写作引擎，5步驱动。Director(LLM)→上下文搜集(零LLM)→骨架Agent(LLM)→渲染(LLM)→状态更新(零LLM)。当用户说"写第N章"/"下一章"/"正文"/"续写章节"时启用。
---
...
| ❌7 | **State after 必须更新** — global-summary + entity-state 维护 | [ ] |
