...
> 起点管线总控。Phase 0→6路由调度。v3.1.0：新增导入/续写模式（Step 0），支持已有历史资料/正文。完整版本历史见CHANGELOG.md。

---

## 做什么

输入：项目名（用户给）或当前项目目录。mode=fresh（从零开始）/import（导入已有设定）/resume（续写已有正文）
输出：标准化目录结构（素材/设计/正文/审核）+ `项目总控.html`（唯一状态文件，含mode字段）
...
### Step 0：导入/续写模式 → `steps/step0-import.md`
检测已有资产→缺口分析→落地Phase决策→状态重建→（如有正文）反推快照。当用户说"导入/续写/已有"或目录已有文件时触发，替代Step 1的从零初始化。

### Step 1：初始化 → `steps/step1.md`
创建项目目录（8个子目录含审核/）+读模板生成项目总控.html+自检11项。**如果检测到已有文件→重定向到Step 0**。
...
12. **导入/续写模式不可跳过资产扫描**——用户说"导入/续写/已有"时必须走Step 0（资产扫描→缺口分析→落地Phase决策→用户确认），禁止直接凭空设置phase。
...
| `steps/step1.md` | 初始化时（state=init且无已有文件） | 目录创建+项目总控.html生成+自检11项 |
...
v3.1.0 | 2026-07-24
- **新增导入/续写模式（Step 0）**：支持用户已有历史资料/设定/正文时跳过从零初始化
- 新增 `steps/step0-import.md`：资产扫描→缺口分析→落地Phase决策→状态重建→正文反推
- step1.md增加前置检测：检测到已有文件→重定向到Step 0
- 项目总控.html模板新增 mode 状态字段（fresh/import/resume）
