...
> 起点管线总控。Phase 0→6路由调度。v1.8.0：Phase 0-1并行（拆书+seed Step 0同时启动）+seed故事先行。
...
| 0-Stage2 | **并行**：①拆书子agent（download/dna-style/research×2） ②seed Step 0交互（S1故事创意→S2力量体系配套） | Stage1完成 | 素材/（调研+文风锚定+decon-lite） + 设计/立项决策表.md（S1-S2部分） |
| 1 | pop-qidian-seed v8.5.0（Step 0续交互S3-S5→骨架层+创意+首章） | Step 0 S1-S2完成+拆书就绪 | 设计/立项决策表.md（完整）+骨架.md+创意.md+正文/ch001.txt |
| 2 | pop-qidian-seed v8.5.0（主角层） | 骨架自洽通过 | 设计/主角设计.md |
| 3 | pop-qidian-world v2.2.0（Step 0交互→世界圣经） | 骨架+主角+ch001就绪 | 设计/世界决策表.md+全书设定/（多文件） |
...
> **Phase 0-1并行设计**（v1.8.0新增）：Stage1深问完成后，拆书子agent和seed Step 0交互**同时启动**。拆书结果（decon-lite/decon-plot）可实时喂给Step 0的S2-S5选项。S1故事创意对齐仅需用户意图.md，不依赖拆书结果，因此可立即开始。S2力量体系配套消费拆书结果，需等拆书返回或用已有信息先生成选项。
...
v1.8.0 | 2026-07-22 → CHANGELOG.md
- **Phase 0-1并行设计**：Stage1深问完成后，拆书子agent和seed Step 0交互同时启动
- **seed故事先行**：S1故事创意对齐先行（仅需用户意图.md），S2力量体系配套（消费拆书结果）
- Phase路由表更新：0-Stage2改为并行①拆书②seed Step 0（S1-S2）
- seed版本更新：v8.4.0→v8.5.0
v1.7.0 | 2026-07-22
