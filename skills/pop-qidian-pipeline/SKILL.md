...
> 起点管线总控。Phase 0→6路由调度。v3.4.0。完整版本历史见CHANGELOG.md。
...
| 0-Stage2 | **并行**：①拆书子agent（download/dna-style/research×2） ②seed Step 0交互（S0→S1世界→S2力量→S3主角） | Stage1完成 | 素材/（调研+文风锚定+decon-lite） + 设计/立项决策表.md |
...
| 3 | pop-qidian-world v3.0.0（交互→**子agent**生成世界圣经） | 骨架+主角+ch001就绪 | 设计/世界决策表.md+全书设定/世界圣经.md |
| 3.5 | pop-qidian-character v1.2.0（交互→**子agent**生成角色库） | 全书设定就绪 | 设计/角色库/角色库.md |
| 4 | pop-qidian-plot v4.3.0（交互→**子agent**生成卷纲+章锚点） | 设定+角色库就绪 | 设计/第一卷剧情/卷纲.md+章锚点表.md |
| 5 | pop-qidian-write v3.5.0（**子agent**） | 剧情+角色库+主角就绪 | 正文/chXXX.txt |
| 6 | pop-qidian-review v3.4.0 | 正文产出 | 审核/review-chXXX.md+小说快照.md |

> Phase 0-1并行设计、产出真实性门禁、下载失败中断、子agent派发模板、Phase 5/6执行细节、Phase 1-4交互模式等规则见 `steps/step2.md`
...
4. **子agent派发必须遵守host约束**——`paopao_subagent_run` 无 `skills` 参数，`purpose` 只支持 `verification/research/critique/implementation-check`。执行类任务必须用 `purpose: "research"`，禁止用 `implementation-check`。Phase 0/3/3.5/4/5的生成任务都必须派发子agent，禁止在主会话直接执行。详细模板见step2.md。
...
| `steps/step2.md` | 每次路由时 | 读html状态→按phase路由→子agent派发模板→更新html |
