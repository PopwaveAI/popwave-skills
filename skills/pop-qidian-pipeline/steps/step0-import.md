...
| Phase 1 | 立项/01-立项PRD.md | 六要素齐全=✅ |
| Phase 3 | 世界决策表.md + 全书设定/ 有文件 + 力量体系.md + 动力引擎.md | 都有=✅ |
| Phase 3→3.5 | 文风锚定.md（含世界画风层） | 存在v2=✅ |
| Phase 3.5 | 金手指.md + 角色库/角色库.md | 都有=✅ |
| Phase 4 | 主线.md + 第一卷剧情/卷纲.md + 章锚点表.md | 都有=✅ |
...
按 `references/html-update-protocol.md` §2（导入重建字段）更新 mode/phase/chapter/project_name/updated_at。

#### 标记已完成Phase circle + 就绪badge

将已就绪的Phase circle从 `pending` 改为 `done`，将落地Phase从 `pending` 改为 `current`。将已有资产对应的badge从❌改为✅。Phase ID对照表和SearchReplace规范见 `references/html-update-protocol.md` §3（单源，此处不复述）。
...
|:--|:--|:--|:--|
| 有正文 + 缺白描卡/状态快照 | review | 批量回溯审核已有正文 → 生成逐章白描卡 + 状态快照 | pop-qidian-review steps/step-reconstruct.md |
| 缺Phase 1-4设计文档 | 对应skill（seed/world/character/plot） | 读取已有文件 → 按skill方法论校验+补全 → 输出标准格式 | 对应skill的SKILL.md+step文件（reconstruct模式） |
...
**主agent必须加载review skill执行**——按review reconstruct模式的SOP执行正文反推（`skills/pop-qidian-review/steps/step-reconstruct.md`）。
...
| 卷纲.md+章锚点表.md | plot | 力量体系+动力引擎+角色库+金手指+全书设定+已有正文 | 标准卷纲+章锚点表 |

> **注意**：如果用户已有正文但缺卷纲+章锚点表，plot reconstruct模式会从正文反推卷纲。如果用户选择跳过plot直接续写（降级模式），需在状态快照或章锚点表中手动指定下一章核心事件，write按章锚点表指导续写。后续可补跑plot生成正式卷纲。
