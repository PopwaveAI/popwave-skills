...
- `素材/` + `素材/downloads/` + `素材/知识沉淀/`
- `设计/` + `设计/全书设定/` + `设计/角色库/` + `设计/第一卷剧情/`
- `产出/` + `产出/白描卡/`（review双文件落盘目录：白描卡+状态快照）
- `正文/`
...
然后按 `references/html-update-protocol.md` §1（初始化创建字段）更新 project_name / created_at / updated_at / genre 四个字段（未指定的保留模板默认值）。
