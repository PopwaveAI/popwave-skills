...
- **skill scope**：当前可用 scope 中是否真实存在 pop-qidian-seed/research/write/review
- **skill version**：pop-qidian/write/review 是否为 DNA-aware 版本；seed/research 是否兼容
- **current-state**：存在 / 缺失 / 过大（超 2500 字）/ 缺字段
- **soul**：存在 / 缺失 / 空泛 / 越权写事实
- **文风DNA闭环**：若启用 DNA，soul 是否有长期融合策略，current-state 是否有本章DNA执行包，DNA源是否存在
- **write 输入闭环**：write 是否只读 current-state + soul + 最近正文 + 用户要求；启用DNA时是否只读执行包指向的DNA源片段
