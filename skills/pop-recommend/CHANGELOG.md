...
**背景**：老板定调——每个 C 端专家需要一份面向给书做安利的博主/作者（非产品经理/AI 专家）的首次对话引导语，用"你能把一本书变成什么"的场景快速建立认知，而非介绍内部架构。

**改动**：

- 新增 `references/onboarding-guide.md`：C 端口吻引导语（一句话说清 + 你有这些玩法 + 你不用担心 + 就这样开始），覆盖原文→推书卡/无剧透安利/一套传播物料
- `SKILL.md`：新增「🚪 首次对话引导」区块——用户首次触发时直接输出引导语全文，再进 Step 1 三阶段扫描
- 版本至 v1.3.0
