...
**根因**: SKILL.md 过长（339行），主流程内联导致维护困难；pipeline upstream 与实际消费方不一致
**类型**: refactor
**改动**:
- **重构**: SKILL.md 从 339 行压缩至 150 行。主工作流迁移至 `steps/` 目录（step-01~05 + step-b-legacy-mode.md）
- **新增**: 红线表（7条硬约束）+ Drop Check（3项执行前必检）
- **新增**: 核心流程表 — 指向 `steps/` 文件
- **变更**: `skill.json` pipeline upstream 从 `pop-reader-making` 改为 `pop-prose-render`（对齐实际消费方）
- **结构**: 设计哲学节压缩为引用指针 → `references/design-philosophy.md`；API Key 节压缩为单表

### 改动文件
- `SKILL.md` — 重构：339行 → 150行，主流程外移 steps/，新增红线表+Drop Check
- `skill.json` — pipeline.upstream: pop-reader-making → pop-prose-render
- `steps/step-01-understand-data.md` — 校验（内容未变）
- `steps/step-02-write-prd.md` — 校验（内容未变）
- `steps/step-03-generate-images.md` — 校验（内容未变）
- `steps/step-04-write-custom-html.md` — 校验（内容未变）
- `steps/step-05-polish.md` — 校验（内容未变）
- `steps/step-b-legacy-mode.md` — 校验（内容未变）
- `CHANGELOG.md` — 本条目
