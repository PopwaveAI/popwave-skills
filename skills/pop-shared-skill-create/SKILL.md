...
> Popwave Skill 设计规范。不是工具，是规范——告诉 agent "skill 该写成什么样、该按什么顺序落盘"。**v7.3.0：去AI味规范补「装饰性比喻黑名单」**（引擎/闭环/全链路等重灾区固化为源头红线）。完整版本历史见 [CHANGELOG.md](CHANGELOG.md)。
...
**装饰性比喻黑名单**（全仓实测重灾区，禁用装饰用法）：

- **禁「XX器/XX引擎」自我定位**：skill 是什么就是什么（`剧情引擎`→剧情推演，`画风测试引擎`→画风测试；术语后缀 `-engine` 保留为技术命名的除外）。
- **禁空洞流程名词**：闭环→完成/收束/循环、全链路→整条流程、拼图/发生器/孵化器/熔炉/淬炼/乐章/画卷/乐高/车间/摸高→删或换直述。
- **禁装饰性比喻链**：不用"构建X生态""打通X链路""结晶沉淀""保驾护航"式包装；改成谁做了什么、产出什么。
...
**当前版本**：v7.3.0 | 2026-08-31 — 去AI味规范补「装饰性比喻黑名单」，重灾区词从源头锁死。完整版本历史见 [CHANGELOG.md](CHANGELOG.md)。
