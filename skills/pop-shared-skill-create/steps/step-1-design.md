...
| ❌D16 | **速查表缺门禁列** — 产出的 skill 速查表必须五列（步骤/必须加载/产出/门禁）。「必须加载」列标明每步执行前必须加载的文件路径，加 ⛔ 前缀表示强制。门禁列标明不加载的后果和退回动作。不是"读什么文件"（建议性），是"必须加载"（强制性） |
...
| ❌D18 | **SKILL.md 缺步骤加载门禁专节** — 有独立 step 文件的 skill，SKILL.md 必须有「⚠️ 步骤加载门禁」专节（在质量红线后、速查表前），列出每个产出物对应的必加载文件 + 跳过后果 + 门禁检查步骤 |
...
> ⚠️ **D16 红线**：速查表必须五列。「必须加载」列不是"读什么文件"（建议性），是"必须加载"（强制性），加 ⛔ 前缀。门禁列标明不加载的后果。
...
有独立 step 文件的 skill，SKILL.md 必须在质量红线后、速查表前加此专节：
...
❌ WRONG: 速查表写"读什么文件"，agent 只看"产出"列直接干 → step 文件从未被加载
✅ CORRECT: 速查表写"必须加载"，加 ⛔ 符号 + 门禁列标明不加载的后果

❌ WRONG: step 文件之间无加载门禁，agent 读完 step-1 就自己发挥完整个流程
✅ CORRECT: 每个 step 文件末尾加「加载下一 step 前禁止产出任何文件」门禁

❌ WRONG: SKILL.md 只有速查表，没有步骤加载门禁专节 → agent 不知道跳过 step 文件有什么后果
✅ CORRECT: SKILL.md 在质量红线后、速查表前加「⚠️ 步骤加载门禁」专节，列出每个产出物的必加载文件和跳过后果
```
