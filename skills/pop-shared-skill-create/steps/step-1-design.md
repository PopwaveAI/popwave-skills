...
├── steps/            ← 可选：分步执行指令
├── templates/        ← 可选：模板文件
├── references/       ← 可选：方法论支撑资料
└── examples/         ← 可选：范例数据
```

❌ 不放 README.md。❌ 不另放 docs/ 目录。
...
| 知识层 | 方法论、模板、规范参考 | references/ | 按需加载 |

**SKILL.md 只做路由。** 红线/错误/边界条件/方法论全部下沉到 steps/ 或 references/。
...
✅ CORRECT: 速查表是全文件目录引导，steps/ 和 references/ 的每个文件都列出读取时机
```
