...
- 设计文档补跑按依赖链顺序：stage（力量体系+动力引擎+全书设定+金手指+角色库+主线+卷一舞台）→ plot（brief→卷纲→幕白描）→ outline（章纲）。用户已有正文但缺剧情文档→plot 从正文反推；用户选跳过 plot 直接续写（降级模式）→在全书日志手动指定下一章核心事件。
...
1. **创建目录（一次性）**：`素材/`（含 `downloads/`、`知识沉淀/`）+ `立项/` + `设计/`（含 `全书设定/`、`角色库/`、`卷舞台/`）+ `卷纲/`（含 `章纲/`）+ `正文/` + `章节日志/`。创建后 LS 自检全部存在，缺一=初始化失败，补创后才继续。
...
5. **路由 Phase 1**：加载 pop-snow-seed 按其 SKILL.md 执行（三输入路径 A/B/C → 与用户共创三层：L1命运图→L2命运图plus→L3全书大纲）。
...
| 2a+ | 卷级调研（轻量，brief有增量需求时跑） | pop-snow-research 模式2 | `素材/卷级调研-卷N.md` | 2b |
...
| Skill | 定位 | 何时调用 |
|:--|:--|:--|
| `pop-snow-seed` | 立项（A/B/C路径→共创三层：L1命运图/L2命运图plus/L3全书大纲） | Phase 1 |
| `pop-snow-stage` | 舞台（首喷=贯穿层+卷一舞台；模式B=卷级刷新） | Phase 2 / 2b |
| `pop-snow-plot` | 剧情（卷需求brief/卷纲/幕白描） | 2a / 2c |
| `pop-snow-outline` | 章纲拼接 | 2d |
| `pop-snow-write` | 正文渲染 | 2e |
| `pop-snow-review` | 审核+沉淀（含 reconstruct 回溯） | 2f / 导入补跑 |
| `pop-snow-research` | 调研（立项调研/卷级调研/wiki盘点+建包） | seed 底牌处理 / 2a+ / 无包时建包 |
