...
1. **创建目录（一次性）**：`素材/`（含 `downloads/`、`知识沉淀/`）+ `立项/` + `设计/`（含 `卷舞台/`）+ `卷纲/`（含 `章纲/`）+ `正文/` + `章节日志/`。创建后 LS 自检全部存在，缺一=初始化失败，补创后才继续。
...
5. **路由 Phase 1**：加载 pop-snow-seed 按其 SKILL.md 执行（六步流水线共创《融合立项稿14维》：问诉求→定调研清单→调研→归集取材·卖点源→融合→定稿交接）。
