...
数据存储在 novel_memory.db 的 pop_arcs 表中
...
class ArcManager:
    ...
    def _ensure_tables(self):
        ...
        """)
        cur.execute("CREATE INDEX IF NOT EXISTS idx_pop_arcs_type ON pop_arcs(arc_type)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_pop_arcs_phase ON pop_arcs(phase_id)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_pop_arcs_start ON pop_arcs(start_chapter)")

        # pop_scenes_content 表用于自动推断最新章节
        cur.execute("""
        ...
        """)
        cur.execute("CREATE INDEX IF NOT EXISTS idx_pop_scenes_chapter ON pop_scenes_content(chapter)")
    ...
    def get_arcs_in_phase(self, phase_id: str) -> List[Dict[str, Any]]:
        ...
        cur = self.conn.cursor()
        cur.execute("""
            SELECT * FROM pop_arcs WHERE arc_type = 'arc' AND phase_id = ?
    ...
    def list_phases(self) -> List[Dict[str, Any]]:
        """列出所有阶段"""
        cur = self.conn.cursor()
        cur.execute("SELECT * FROM pop_arcs WHERE arc_type = 'phase' ORDER BY start_chapter")
        return [dict(row) for row in cur.fetchall()]
    ...
    def list_arcs(self, phase_id: str = None) -> List[Dict[str, Any]]:
        ...
        if phase_id:
            ...
            cur.execute("SELECT * FROM pop_arcs WHERE arc_type = 'arc' ORDER BY start_chapter")
        return [dict(row) for row in cur.fetchall()]
    ...
    def complete_arc(self, arc_id: str, end_chapter: int) -> bool:
        ...
        cur = self.conn.cursor()
        cur.execute("""
            UPDATE pop_arcs SET end_chapter = ?, updated_at = ? WHERE id = ?
        """, (end_chapter, now, arc_id))
        self.conn.commit()
        return cur.rowcount > 0

    def get_latest_chapter(self) -> int:
        """从 pop_scenes_content 表查询已存储的最新章节号"""
        cur = self.conn.cursor()
        try:
