...
class ContextRetriever:
    ...
    def _get_book_summary(self) -> str:
        ...
        cur.execute("SELECT content FROM pop_summaries WHERE level='book' AND range_desc='全书'")
        row = cur.fetchone()
    ...
    def _get_current_state(self) -> str:
        """L0: 当前状态（滚动更新）"""
        cur = self.conn.cursor()
        cur.execute("SELECT content FROM pop_summaries WHERE level='book' AND range_desc='current_state'")
        row = cur.fetchone()
        return row["content"] if row else ""
    ...
    def _get_phase_summary(self, chapter: int) -> str:
        ...
        cur = self.conn.cursor()
        cur.execute("""
            SELECT s.content FROM pop_summaries s
            JOIN pop_arcs a ON s.range_desc = a.id
            WHERE s.level = 'phase'
    ...
    def _get_volume_summary(self, volume: int) -> str:
        """L3: 卷摘要"""
        cur = self.conn.cursor()
        cur.execute("SELECT content FROM pop_summaries WHERE level='volume' AND range_desc=?",
                   (str(volume),))
        row = cur.fetchone()
        return row["content"] if row else ""
    ...
    def _get_chapter_summary(self, chapter: int) -> str:
        """L4: 章节摘要"""
        cur = self.conn.cursor()
        cur.execute("SELECT content FROM pop_summaries WHERE level='chapter' AND range_desc=?",
                   (str(chapter),))
        row = cur.fetchone()
        return row["content"] if row else ""
    ...
    def _get_related_scenes(self, query: str, chapter: int,
        ...
        try:
            ...
            try:
                ...
                cur = self.conn.cursor()
                cur.execute("""
                    SELECT start_chapter, end_chapter FROM pop_arcs
