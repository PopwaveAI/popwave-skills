...
class FactEngine:
    ...
    def _ensure_tables(self):
        ...
        for col, default in [("importance", "'chapter-scoped'"),
            ...
            try:
                cur.execute(f"ALTER TABLE pop_facts ADD COLUMN {col} {'TEXT' if col == 'importance' else 'INTEGER'} DEFAULT {default}")
    ...
    def set_fact(self, category: str, entity: str, attribute: str,
        ...
        try:
            ...
            cur.execute("""
                UPDATE pop_facts SET superseded_by = -1
                WHERE entity = ? AND attribute = ? AND superseded_by IS NULL
            """, (entity, attribute))

            cur.execute("""
                INSERT INTO pop_facts (category, entity, attribute, value, chapter, importance,
            ...
            # 将临时标记替换为真正的新事实ID
            cur.execute("""
                UPDATE pop_facts SET superseded_by = ?
            ...
            try:
                self.conn.execute(
                    "UPDATE pop_facts SET superseded_by = NULL WHERE superseded_by = -1"
                )
                self.conn.commit()
            except Exception:
                pass
    ...
    def get_all_facts(self, entity: str = None, category: str = None,
        ...
        """获取所有当前有效的事实（可选限制数量）"""
        sql = "SELECT * FROM pop_facts WHERE superseded_by IS NULL"
        params = []
        if entity:
            sql += " AND entity = ?"
    ...
    def get_relevant_facts(self, current_chapter: int,
        ...
        active_condition = """
            (superseded_by IS NULL
             OR id IN (
                 SELECT f1.id FROM pop_facts f1
                 JOIN pop_facts f2 ON f1.superseded_by = f2.id
    ...
    def archive_old_facts(self, current_chapter: int, threshold: int = 200) -> int:
        ...
        try:
            ...
            cur.execute("""
                UPDATE pop_facts SET superseded_by = 0
                WHERE superseded_by IS NULL
                AND importance = 'chapter-scoped'
                AND chapter < ?
    ...
    def supersede_chapter_facts(self, chapter: int) -> int:
        ...
        try:
            ...
            cur.execute("""
                UPDATE pop_facts SET superseded_by = -2
                WHERE superseded_by IS NULL
                AND chapter = ?
            """, (chapter,))
            count = cur.rowcount
    ...
    def get_latest_chapter(self) -> int:
        """获取项目中已存储的最新章节号（从 pop_scenes_content 查询）"""
        cur = self.conn.cursor()
        cur.execute("SELECT MAX(chapter) FROM pop_scenes_content")
        row = cur.fetchone()
    ...
    def execute_action(self, action: str, params: dict) -> dict:
        ...
        if action == "set-fact":
        ...
        elif action == "get-pop_facts":
            return {"facts": self.get_all_facts(params.get("entity"),
                                                 params.get("category"),
                                                 params.get("limit"))}

        elif action == "relevant-pop_facts":
            chapter = params.get("chapter")
            if chapter is None:
            ...
            return {"facts": pop_facts, "count": len(facts), "current_chapter": int(chapter)}
            ...
            if attribute:
                ...
                return {"entity": entity, "pop_facts": self.get_all_facts(entity)}

        elif action == "archive-pop_facts":
            chapter = params.get("chapter")
            if chapter is None:
        ...
        elif action == "supersede-chapter-pop_facts":
            chapter = params.get("chapter")
            if chapter is None:
