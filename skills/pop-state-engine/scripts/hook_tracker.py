...
数据存储在 novel_memory.db 的 pop_hooks 表中
支持: 种埋、收线、遗忘预警、超期统计
"""
...
class HookTracker:
    ...
    def _ensure_tables(self):
        ...
        """)
        cur.execute("CREATE INDEX IF NOT EXISTS idx_pop_hooks_status ON pop_hooks(status)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_pop_hooks_priority ON pop_hooks(priority)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_pop_hooks_planted ON pop_hooks(planted_chapter)")
        self.conn.commit()
    ...
    def plant(self, desc: str, planted_chapter: int,
        ...
        existing = self.conn.execute(
            "SELECT id FROM pop_hooks WHERE desc = ? AND status = 'open'", (desc,)
        ).fetchone()
        if existing:
            return existing["id"]
        ...
        cur = self.conn.cursor()
        cur.execute("""
            INSERT OR IGNORE INTO pop_hooks (id, desc, planted_chapter, expected_resolve,
    ...
    def resolve(self, hook_id: str, resolved_chapter: int, resolution: str = "") -> bool:
        ...
        cur = self.conn.cursor()
        cur.execute("SELECT status FROM pop_hooks WHERE id=?", (hook_id,))
        row = cur.fetchone()
        if not row:
            return False
    ...
    def get_open_hooks(self, current_chapter: int = None,
        ...
        """获取未收线的伏笔"""
        cur = self.conn.cursor()
        sql = "SELECT * FROM pop_hooks WHERE status = 'open'"
        params = []
    ...
    def get_hook(self, hook_id: str) -> Optional[Dict[str, Any]]:
        """获取单个伏笔"""
        cur = self.conn.cursor()
        cur.execute("SELECT * FROM pop_hooks WHERE id = ?", (hook_id,))
        row = cur.fetchone()
        if not row:
            return None
        r = dict(row)
    ...
    def list_all(self, status: str = None) -> List[Dict[str, Any]]:
        ...
        cur = self.conn.cursor()
        if status:
            cur.execute("SELECT * FROM pop_hooks WHERE status = ? ORDER BY planted_chapter", (status,))
        else:
            cur.execute("SELECT * FROM pop_hooks ORDER BY planted_chapter")
        results = []
        for row in cur.fetchall():
            r = dict(row)
            r["tags"] = json.loads(r.get("tags", "[]"))
    ...
    def get_stats(self) -> Dict[str, Any]:
        """获取伏笔统计"""
        cur = self.conn.cursor()
        cur.execute("SELECT status, COUNT(*) FROM pop_hooks GROUP BY status")
        status_counts = {row[0]: row[1] for row in cur.fetchall()}
        cur.execute("SELECT priority, COUNT(*) FROM pop_hooks WHERE status = 'open' GROUP BY priority")
