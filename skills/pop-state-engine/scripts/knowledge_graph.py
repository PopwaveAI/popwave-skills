...
数据存储在 novel_memory.db 的 pop_kg_nodes / pop_kg_edges 表中
"""
...
class KnowledgeGraph:
    ...
    def _ensure_tables(self):
        ...
        """)
        cur.execute("CREATE INDEX IF NOT EXISTS idx_pop_pop_kg_nodes_type ON pop_kg_nodes(type)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_pop_pop_kg_nodes_name ON pop_kg_nodes(name)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_pop_pop_kg_edges_source ON pop_kg_edges(source)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_pop_pop_kg_edges_target ON pop_kg_edges(target)")
        self.conn.commit()
    ...
    def add_node(self, name: str, node_type: str, tags: List[str] = None,
        ...
        cur = self.conn.cursor()
        cur.execute("""
            INSERT OR REPLACE INTO pop_kg_nodes (id, type, name, tags, properties, created_at)
            VALUES (?, ?, ?, ?, ?, ?)
    ...
    def get_node(self, node_id: str) -> Optional[Dict[str, Any]]:
        """获取节点"""
        cur = self.conn.cursor()
        cur.execute("SELECT * FROM pop_kg_nodes WHERE id = ?", (node_id,))
        row = cur.fetchone()
        if not row:
            return None
        result = dict(row)
    ...
    def find_node_by_name(self, name: str) -> Optional[Dict[str, Any]]:
        """按名称查找节点"""
        cur = self.conn.cursor()
        cur.execute("SELECT * FROM pop_kg_nodes WHERE name = ?", (name,))
        row = cur.fetchone()
        if not row:
            return None
        result = dict(row)
    ...
    def list_nodes(self, node_type: str = None, tag: str = None) -> List[Dict[str, Any]]:
        ...
        cur = self.conn.cursor()
        if node_type:
            cur.execute("SELECT * FROM pop_kg_nodes WHERE type = ? ORDER BY name", (node_type,))
        elif tag:
            cur.execute("SELECT * FROM pop_kg_nodes WHERE tags LIKE ? ORDER BY name",
                       (f'%"{tag}"%',))
        else:
            cur.execute("SELECT * FROM pop_kg_nodes ORDER BY type, name")
        results = []
    ...
    def update_node(self, node_id: str, name: str = None, node_type: str = None,
        ...
        cur = self.conn.cursor()
        if name is not None:
            cur.execute("UPDATE pop_kg_nodes SET name = ? WHERE id = ?", (name, node_id))
        if node_type is not None:
            cur.execute("UPDATE pop_kg_nodes SET type = ? WHERE id = ?", (node_type, node_id))
        if tags is not None:
            cur.execute("UPDATE pop_kg_nodes SET tags = ? WHERE id = ?",
                       (json.dumps(tags, ensure_ascii=False), node_id))
        if properties is not None:
            cur.execute("UPDATE pop_kg_nodes SET properties = ? WHERE id = ?",
    ...
    def delete_node(self, node_id: str) -> bool:
        """删除节点及其所有关系"""
        cur = self.conn.cursor()
        cur.execute("DELETE FROM pop_kg_edges WHERE source = ? OR target = ?", (node_id, node_id))
        cur.execute("DELETE FROM pop_kg_nodes WHERE id = ?", (node_id,))
        self.conn.commit()
    ...
    def add_edge(self, source: str, target: str, relation: str,
        ...
        cur = self.conn.cursor()
        cur.execute("""
            INSERT INTO pop_kg_edges (source, target, relation, properties, created_at)
            VALUES (?, ?, ?, ?, ?)
    ...
    def get_edges(self, node_id: str, direction: str = "both",
        ...
        sql = f"SELECT * FROM pop_kg_edges WHERE ({' OR '.join(conditions)})"
        if relation:
            sql += " AND relation = ?"
    ...
    def get_graph_stats(self) -> Dict[str, Any]:
        ...
        cur.execute("SELECT type, COUNT(*) FROM pop_kg_nodes GROUP BY type")
        type_counts = {row[0]: row[1] for row in cur.fetchall()}
        cur.execute("SELECT relation, COUNT(*) FROM pop_kg_edges GROUP BY relation")
