...
class MemoryEngine:
    ...
    def _segment_for_fts(text: str) -> str:
        """将文本用 bigram（双字）分词后以空格连接，使 FTS5 unicode61 能正确索引中文
        
        bigram 是 CJK 搜索的标准技术：
        - "碎骨指" → "碎骨 骨指"（两个 bigram，AND 匹配 ≈ 子串匹配）
        - "碎骨指功法" → "碎骨 骨指 指功 功法"
        - 索引和查询使用相同算法，保证一致性
        - 单字符直接作为 token
        - ASCII 文本保持原样（unicode61 已正确处理）
        """
        if not text:
            return text
                ...
                if '\u4e00' <= ch <= '\u9fff' or '\u3400' <= ch <= '\u4dbf':
                    ...
                    if current_cjk:
                        ...
                        if len(current_cjk) == 1:
                            result.append(current_cjk)
                        else:
                            for i in range(len(current_cjk) - 1):
                                result.append(current_cjk[i:i+2])
                        current_cjk = ""
                    # 非 CJK 字符累积为 ASCII token
                    if ch.isalnum():
                        result.append(ch)
            if current_cjk:
                if len(current_cjk) == 1:
    ...
    def _init_schema(self):
        ...
        # 版本表
        cur.execute("""
            CREATE TABLE IF NOT EXISTS pop_schema_version (
                version INTEGER PRIMARY KEY
            )
        """)

        # 读取当前版本
        cur.execute("SELECT MAX(version) FROM pop_schema_version")
        row = cur.fetchone()
        ...
        if current_version < 1:
            cur.execute("""
                CREATE VIRTUAL TABLE IF NOT EXISTS pop_scenes USING fts5(
                    content, tags, chapter, location, characters, mood, events,
                    content='pop_scenes_content', tokenize='unicode61'
                )
            """)
            cur.execute("""
                CREATE TABLE IF NOT EXISTS pop_scenes_content (
            ...
            """)
            cur.execute("""
                CREATE TABLE IF NOT EXISTS pop_embeddings (
                    scene_id INTEGER PRIMARY KEY,
                    embedding BLOB,
                    model_name TEXT,
                    FOREIGN KEY (scene_id) REFERENCES pop_scenes_content(id)
                )
            """)
            cur.execute("""
                CREATE TABLE IF NOT EXISTS pop_summaries (
                    id INTEGER PRIMARY KEY,
                    level TEXT NOT NULL,
            ...
            """)
            cur.execute("""
                CREATE TABLE IF NOT EXISTS pop_facts (
                    id INTEGER PRIMARY KEY,
                    ...
                    FOREIGN KEY (superseded_by) REFERENCES pop_facts(id)
                )
            """)
            cur.execute("""
                CREATE INDEX IF NOT EXISTS idx_pop_facts_entity_attr
                ON pop_facts(entity, attribute)
            """)
            cur.execute("""
                CREATE INDEX IF NOT EXISTS idx_pop_facts_active
                ON pop_facts(entity, attribute) WHERE superseded_by IS NULL
            """)
        ...
        if current_version < 2:
            # 事实表扩展: 相关性引擎
            try:
                cur.execute("ALTER TABLE pop_facts ADD COLUMN importance TEXT DEFAULT 'chapter-scoped'")
            except sqlite3.OperationalError:
                pass
            try:
                cur.execute("ALTER TABLE pop_facts ADD COLUMN valid_from INTEGER")
            except sqlite3.OperationalError:
                pass
            try:
                cur.execute("ALTER TABLE pop_facts ADD COLUMN valid_until INTEGER")
            except sqlite3.OperationalError:
                pass
            cur.execute("""
                CREATE INDEX IF NOT EXISTS idx_pop_facts_importance
                ON pop_facts(importance) WHERE superseded_by IS NULL
            """)
            cur.execute("""
                CREATE INDEX IF NOT EXISTS idx_pop_facts_valid_range
                ON pop_facts(valid_from, valid_until) WHERE superseded_by IS NULL
            """)

            # 场景表索引: 章节范围查询
            cur.execute("""
                CREATE INDEX IF NOT EXISTS idx_pop_scenes_chapter
                ON pop_scenes_content(chapter)
            """)

            # 知识图谱表
            cur.execute("""
                CREATE TABLE IF NOT EXISTS pop_kg_nodes (
                    id TEXT PRIMARY KEY,
                    type TEXT NOT NULL,
                    name TEXT NOT NULL,
                    tags TEXT,
            ...
            """)
            cur.execute("""
                CREATE TABLE IF NOT EXISTS pop_kg_edges (
                    id INTEGER PRIMARY KEY,
                    ...
                    properties TEXT,
                    created_at TEXT,
                    FOREIGN KEY (source) REFERENCES pop_kg_nodes(id),
                    FOREIGN KEY (target) REFERENCES pop_kg_nodes(id)
                )
            """)
            cur.execute("""
                CREATE INDEX IF NOT EXISTS idx_pop_pop_kg_nodes_type ON pop_kg_nodes(type)
            """)
            cur.execute("""
                CREATE INDEX IF NOT EXISTS idx_pop_pop_kg_nodes_name ON pop_kg_nodes(name)
            """)
            cur.execute("""
                CREATE INDEX IF NOT EXISTS idx_pop_pop_kg_edges_source ON pop_kg_edges(source)
            """)
            cur.execute("""
                CREATE INDEX IF NOT EXISTS idx_pop_pop_kg_edges_target ON pop_kg_edges(target)
            """)

            # 伏笔表
            cur.execute("""
                CREATE TABLE IF NOT EXISTS pop_hooks (
                    id TEXT PRIMARY KEY,
                    desc TEXT NOT NULL,
            ...
            """)
            cur.execute("""
                CREATE INDEX IF NOT EXISTS idx_pop_hooks_status ON pop_hooks(status)
            """)
            cur.execute("""
                CREATE INDEX IF NOT EXISTS idx_pop_hooks_planted ON pop_hooks(planted_chapter)
            """)

            # 弧线表
            cur.execute("""
                CREATE TABLE IF NOT EXISTS pop_arcs (
                    id TEXT PRIMARY KEY,
                    title TEXT NOT NULL,
            ...
            """)
            cur.execute("""
                CREATE INDEX IF NOT EXISTS idx_pop_arcs_type ON pop_arcs(arc_type)
            """)
            cur.execute("""
                CREATE INDEX IF NOT EXISTS idx_pop_arcs_range ON pop_arcs(start_chapter, end_chapter)
            """)

        # 记录版本
        if current_version < self.SCHEMA_VERSION:
            cur.execute("DELETE FROM pop_schema_version")
            cur.execute("INSERT INTO pop_schema_version VALUES (?)", (self.SCHEMA_VERSION,))
    ...
    def store_scene(self, content: str, chapter: int, tags: str = "",
        ...
        cur.execute("""
            INSERT INTO pop_scenes_content (content, tags, chapter, location, characters, mood, events)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (content, tags, chapter, location, characters, mood, events))
        ...
        cur.execute("""
            INSERT INTO pop_scenes (rowid, content, tags, chapter, location, characters, mood, events)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ...
        if ONNX_AVAILABLE:
            ...
            if embedding is not None:
                cur.execute("""
                    INSERT INTO pop_embeddings (scene_id, embedding, model_name)
                    VALUES (?, ?, ?)
    ...
    def delete_chapter_scenes(self, chapter_num: int) -> int:
        ...
        # 1. 获取该章节所有场景ID
        cur.execute("SELECT id FROM pop_scenes_content WHERE chapter = ?", (chapter_num,))
        ...
        cur.execute(f"DELETE FROM pop_embeddings WHERE scene_id IN ({placeholders})", scene_ids)

        # 3. 删除 FTS 索引（rowid 与 pop_scenes_content.id 一致）
        cur.execute(f"DELETE FROM pop_scenes WHERE rowid IN ({placeholders})", scene_ids)

        # 4. 删除场景内容
        cur.execute("DELETE FROM pop_scenes_content WHERE chapter = ?", (chapter_num,))
    ...
    def _keyword_search(self, query: str, top_k: int = 5,
        ...
        查询构建策略：
        - 用户输入按空格分词，词间 OR（搜索任一词）
        - 每个词内 bigrams 用 AND 连接（≈ 子串匹配，高精度）
        - 用户显式 " OR " 也支持
        """
        if " OR " in query:
            # 用户显式 OR 查询
            ...
            for part in parts:
                ...
                if tokens:
                    term_queries.append(" AND ".join(tokens))
            fts_query = " OR ".join(term_queries)
        else:
            # 按空格分词，词间 OR，词内 bigram AND
            terms = [t.strip() for t in query.split() if t.strip()]
            term_queries = []
            for term in terms:
                seg = self._segment_for_fts(term)
                tokens = [t.strip() for t in seg.split() if t.strip()]
                if tokens:
                    term_queries.append(" AND ".join(tokens))
            fts_query = " OR ".join(term_queries)
        ...
        sql = """
            SELECT sc.id, sc.content, sc.tags, sc.chapter,
                   ...
                   bm25(pop_scenes) as rank
            FROM pop_scenes
            JOIN pop_scenes_content sc ON pop_scenes.rowid = sc.id
            WHERE pop_scenes MATCH ?
        """
        params = [fts_query]
    ...
    def _semantic_search(self, query: str, top_k: int = 5,
        ...
        sql = """SELECT e.scene_id, e.embedding, sc.content, sc.tags, sc.chapter,
                        sc.location, sc.characters, sc.mood, sc.events
                 FROM pop_embeddings e JOIN pop_scenes_content sc ON e.scene_id = sc.id WHERE 1=1"""
        params = []
    ...
    def store_summary(self, level: str, range_desc: str, content: str) -> int:
        ...
        cur.execute("""
            INSERT OR REPLACE INTO pop_summaries (level, range_desc, content, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?)
    ...
    def get_summary(self, level: str, range_desc: str) -> Optional[str]:
        """获取摘要"""
        cur = self.conn.cursor()
        cur.execute("SELECT content FROM pop_summaries WHERE level=? AND range_desc=?", (level, range_desc))
        row = cur.fetchone()
    ...
    def create_arc(self, arc_id: str, title: str, arc_type: str = "arc",
        ...
        cur = self.conn.cursor()
        cur.execute("""
            INSERT OR REPLACE INTO pop_arcs (id, title, arc_type, start_chapter, end_chapter,
    ...
    def get_arc(self, arc_id: str) -> Optional[Dict[str, Any]]:
        """获取弧线信息"""
        cur = self.conn.cursor()
        cur.execute("SELECT * FROM pop_arcs WHERE id = ?", (arc_id,))
        row = cur.fetchone()
    ...
    def list_arcs(self, arc_type: str = None) -> List[Dict[str, Any]]:
        ...
        cur = self.conn.cursor()
        if arc_type:
            cur.execute("SELECT * FROM pop_arcs WHERE arc_type = ? ORDER BY start_chapter", (arc_type,))
        else:
            cur.execute("SELECT * FROM pop_arcs ORDER BY start_chapter")
        return [dict(row) for row in cur.fetchall()]
    ...
    def update_arc(self, arc_id: str, **kwargs) -> bool:
        ...
        vals.append(arc_id)
        cur = self.conn.cursor()
        cur.execute(f"UPDATE pop_arcs SET {', '.join(sets)} WHERE id = ?", vals)
        self.conn.commit()
        return True
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        cur = self.conn.cursor()
        stats = {}
        cur.execute("SELECT COUNT(*) FROM pop_scenes_content")
        stats["total_scenes"] = cur.fetchone()[0]
        cur.execute("SELECT COUNT(*) FROM pop_summaries")
        stats["total_summaries"] = cur.fetchone()[0]
        cur.execute("SELECT COUNT(*) FROM pop_facts WHERE superseded_by IS NULL")
        stats["active_facts"] = cur.fetchone()[0]
        cur.execute("SELECT COUNT(*) FROM pop_embeddings")
        stats["total_embeddings"] = cur.fetchone()[0]
        cur.execute("SELECT DISTINCT chapter FROM pop_scenes_content ORDER BY chapter")
        stats["chapters"] = [row[0] for row in cur.fetchall()]
        cur.execute("SELECT COUNT(*) FROM pop_kg_nodes")
        stats["pop_kg_nodes"] = cur.fetchone()[0]
        cur.execute("SELECT COUNT(*) FROM pop_kg_edges")
        stats["pop_kg_edges"] = cur.fetchone()[0]
        cur.execute("SELECT COUNT(*) FROM pop_hooks WHERE status = 'open'")
        stats["open_hooks"] = cur.fetchone()[0]
        cur.execute("SELECT COUNT(*) FROM pop_arcs")
    ...
    def execute_action(self, action: str, params: dict) -> dict:
        ...
        elif action == "delete-chapter-pop_scenes":
            chapter = params.get("chapter")
            if chapter is None:
...
def main():
    ...
    parser.add_argument('--action', required=True,
                       choices=['store-chapter', 'store-scene', 'search',
                               ...
                               'delete-chapter-pop_scenes', 'chapter-complete'],
                       help='操作类型')
