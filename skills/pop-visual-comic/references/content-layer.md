...
## 一、三段流程（先定整页 → 再排格 → 再填格内）

```
① 整页系统（本文件§二）→ ② 排版结构（layout-baseline选YZ-1~8）→ ③ 每格镜语（本文件§三-§五）
```

**核心顺序：先定整页叙事，再选排版，最后进格内镜语。** 不得反向——先排格子再想整页，会变成格子罗列。
...
> **S级页不得使用 C1（三分法）**——S级页需要冲击不是安全。名场面从{C2/C4/C6/C9/C10}中选。相邻格构图必须变化，连续2格相同构图=单调。
...
> 想让这格传达某情绪，就从这里查推荐 L/C/F 组合，**不得凭感觉自由发挥**。
...
> 解决"丢失的对话文案"痛点。Seedream 不画文字，文字全部交给 HTML 后处理，但**说话者、情绪、位置须在导演卡中提前固定**。
...
## 六、文字控制（Text Control）—— 不得出现伪对话乱字，保留环境装饰字

> **2026-08-03 实测、老板确认（见 `workspace/文字控制测试/pe-log-2026-08-03.md`）：Seedream 生成的文字（伪汉字、数字）基本不可用。** 但**环境文字（日记、书本、招牌、符咒）的乱字可作为装饰保留**；真正要防的是**"伪对话场景"的乱字**——对话气泡、对话框里放入乱字，完全不可用且无法修复。
...
> **负面词须"逐字列举"**（letters/numbers/words/typography 等），比笼统的 `No text` 有效得多。**对话场景须显式禁用 `speech bubbles / dialogue balloons / dialogue text / sound effect text`**，否则可能画气泡塞乱字（R3 实测）。**须显式禁用 `dial numerals` 与 `roman numerals`**，否则表盘、钟面会生成数字（R1 实测）。
...
- **不得在内容层出现** `handwriting / label / banner / sign / writing / text / numbers` 等会**主动触发**文字渲染的词——除非你希望它作为环境装饰乱字出现。
- **尤其不得**：对话气泡、对话框、拟声字出现在画面（由 HTML 承载）。
...
| 对话气泡乱字 | 画面出现气泡且塞乱字 | ❌ 不得出现，由 HTML 承载 |
