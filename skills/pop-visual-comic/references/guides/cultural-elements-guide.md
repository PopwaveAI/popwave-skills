...
**文化元素用于提升漫画的文化底蕴——将漫画从"插画集合"升级为有文化底蕴的视觉作品。** 不得每页都加，只在该加时加，服务于叙事和氛围。
...
| 功能 | 营销钩子、格调锚点 | 叙事装饰、氛围强化、章节仪式感 |
| 文字叠加 | 提示词直出（Seedream渲染） | HTML 层实现（CSS精确控制）与提示词层（道具铭文） |
...
> **使用纪律**：一章最多使用 2-3 种文化元素，过多会杂乱。文化元素服务于叙事氛围，不得为了"有文化"而硬加。
...
**创作要求**：
1. 2-4 句，每句 5-7 字（五言或七言），总计不超过 28 字
2. 须包含本章的核心意象（道具、场景、情感），不得是通用诗句
3. 押韵但不强求平仄，重意境不重格律
4. 读者扫一眼能感受到"这一章有底蕴"
...
> **字体层级**：由大到小依次为书名（毛笔书法体，最大）、定场诗（宋体或楷体，中等）、副标题（楷体，小）、印章（篆刻体，小但醒目，红色）。
...
| 章末钩子页 | 不加——钩子页要留悬念，加诗会冲淡悬念 | ❌ 不得 |
| 大单页 | 不加——名场面画面自说，文字会干扰 | ❌ 不得 |
...
用古籍印刷的版式元素做章节标题页的边框与装饰，增加"典籍感"。适合修仙、古言、历史赛道。

### 3.2 版式元素

| 元素 | 形态 | 漫画用途 |
|:-----|:-----|:---------|
| **版框线** | 双线或单线矩形框 | 章节标题页边框装饰 |
| **鱼尾** | ♣形装饰符号 | 版框中线两端 |
| **界栏线** | 竖向分隔线 | 定场诗竖排分隔 |
...
仅在章节标题页使用。其他页面不加版式线——在连续翻页阅读中，版式线反复出现会变成视觉噪音。
...
- 不需要模型写出正确的小篆，只需要"看起来像古文字"的视觉效果
...
| 元素 | 实现层 | 原因 |
|:-----|:-------|:-----|
| 道具铭文 | 提示词层 | 铭文在画面中，须由 Seedream 直出 |
...
| 言情甜宠 | 印章（小清新变体）、落款 | 定场诗 | 印章用粉色或浅色变体 |
...
> **核心原则**：文化元素是锦上添花，不是必选项。如果章节内容本身不支撑（如现代都市章），不如不加。