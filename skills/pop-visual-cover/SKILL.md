...
> 网文封面与场景视觉资产生成器。两种起点，同一个终点。v1.6.0
...
- **角色本体身份以 `pop-visual-character` 产出的「角色视觉身份卡」为唯一真源**：封面只要出现角色，就读取 `素材/视觉资产/[角色名]视觉身份卡.md` 作为角色本体（剪影/色彩/细节/记忆锚点/冻结提示词），本模式只负责把角色放进封面构图，**禁止重建角色身份**。身份卡缺失时提示先跑 `pop-visual-character`。
- **档位说明（cover 意图 = 轻量基建）**：Pipeline 语境下 cover 只需基建到身份卡即可派生，**不强制双角度定妆**；如需角色参考图，用 character 产出的**单张**定妆图作图生图参考即可。
