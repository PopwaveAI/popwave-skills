...
> 网文角色立绘生成器。角色调研→设计方案→生成。v2.0.0

## 做什么

输入：小说项目文件 + 角色名 + 可选参考图。
输出：角色档案 + OC立绘图（基础版/加强版/**设定卡传播版**）+ 设计方案文档 + 提示词记录。
...
2. 角色档案 →（选档位+设计信息架构+可选Pinterest参考）→ **门禁B：方案对齐**
...
- 设计信息架构：加强版=六层信息（角色名+身份→称号→角色小传→题诗→书法大字→钤印）；**设定卡传播版=模块化信息（属性栏/配色板/表情差分/花语/境界徽章/语录）**
- 选择布局模板（底部面板/侧边条/叠加层/四角分布/右侧古籍版式/**设定卡模块化版式**）
- **🚪 门禁B**：用户确认设计方案
