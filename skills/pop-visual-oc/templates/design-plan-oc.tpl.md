...
| 顶部标题区 | 系列名 {{series}} / 类型标签 {{type}} / 实体名 {{entity_name}}（鎏金大字）/ 定位线 {{positioning}}（≤14字，`[核心元素]即[本质] · [命运/结果]`） | 图框上方 |
| 信息区-箴言 | {{creed}}（钩子，≤16字，有代价/反转，是台词非介绍句） | 压图下缘，金光 |
| 信息区-核心气质 | {{core_temperament}}（机制人话，可复述，无黑话） | 图框外 |
| 信息区-等级/色板 | {{level_palette}}（层级递进链，≤5字/层） | 图框外 |
| 信息区-象征 | {{symbols}}（两栏词条，键=具象物，值=意味着什么，≤4字/项） | 图框外 |
| 品牌水印 | popwave.cn 让创意一键落地 | footer（已内嵌，不走 watermark.py） |

> 文案按 `references/mode-album-html.md` §文案设计规范 公式填写，对标「借香他化」成功成品。**组装前必过文案稳定性四问**（有钩子/讲清机制/无黑话/圈外人3秒懂），任一不通过回写修正。
