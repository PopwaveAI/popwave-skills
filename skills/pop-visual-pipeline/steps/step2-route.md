...
> **建档层面**：`cover`/`oc` 意图时，基建只需到"美术设定集"（phase2）即可派生，**不强制跑 phase2 的完整复现（双角度定妆/场景定妆）**（那是 comic 完整档才需要）。`comic` 意图才走完整基建（深度资产+画风定标+美术设定集+完整复现）。
...
- **基建层（phase0-2）**：按顺序推进，直到达到 intent 档位所需深度
  - `cover`/`oc`：到 phase2（美术设定集）即基建完成，可派生
  - `comic`/`full`：到 phase2 且完整复现完成（基建完整）才派生
...
- 基建就绪门禁强制，派生层前必须验证美术设定集存在且签核 ✅ 已认可
