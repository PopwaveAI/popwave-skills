...
> 视觉 skill 群的**共享底层资产层**。v1.7.0。本 skill 不独立运行，而是被其他视觉 skill 以"引用共享组件"的方式调用。**生图不直连 API、不内置任何 API Key**，统一由脚本导出 `generation_tasks.json`，再由主 agent 用 `image_generate` 工具逐条生成。
