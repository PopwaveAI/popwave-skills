...
> **铁律：所有出图总像素必须 ≤ 236 万（Seedream 5.0 Pro 计费临界，超 236 万像素报价从 0.3 元/张翻倍到 0.6 元/张）。** 上表全部在安全范围内（最大 1500x1500=225 万）。脚本 `generate.py` 已内置 `assert_size_safe` 校验，超限直接报错中止。分辨率档位仅支持 `1K`（1024x1024=105 万）；`2K`/`4K` 均超上限会被脚本拒绝，禁止使用。
