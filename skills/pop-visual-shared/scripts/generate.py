#!/usr/bin/env python3
"""
pop-visual-shared API 调用脚本（视觉 skill 群共享）
支持 Seedream（图片生成）和 Seedance（视频生成）

用法:
  图片生成:
    python generate.py image --prompt "提示词" --model doubao-seedream-4-5-251128 --size 1125x1500 --output "封面.png"
...
# 官方临界 = 236 万像素：超过后输出图从 0.3 元/张翻倍到 0.6 元/张。
# 铁律：所有出图尺寸总像素必须 ≤ MAX_PIXELS，超限直接报错中止，防止报价翻倍。
MAX_PIXELS = 2360000
...
def assert_size_safe(size):
    ...
    支持 "WxH" 具体像素（如 1500x1500）与档位名（1K/2K/4K）。
    """
    if not size:
        return
...
def main():
    parser = argparse.ArgumentParser(
        description="pop-visual-shared API 调用脚本",
    ...
    img_parser.add_argument("--size", default=None, help="图片尺寸（如 1125x1500，总像素须 ≤236万，超限报错中止）")
    ...
    img_parser.add_argument("--response-format", choices=["url", "b64_json"], default="b64_json", help="返回格式（默认b64_json，避免临时URL过期导致对话流与落盘不一致）")
