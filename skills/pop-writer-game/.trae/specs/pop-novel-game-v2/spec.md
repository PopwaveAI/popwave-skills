# pop-writer-game v2 升级 — 产品需求文档

## Overview

对 pop-writer-game 技能进行一次完整性升级。核心方向：修复当前方法文档中 `stream: true` 与 `response_format: json_object` 不兼容的 bug，引入完整的流式输出 + 动态渲染 UX 设计（AbortController 取消、超时兜底、指数退避重试、动态叙事面板渐进渲染），统一三份方法文档的代码规范，并新增 Spec 三文件作为项目规范底座。
...
pop-writer-game 是 pop 技能体系中负责「小说世界观 → AI 文游」转化的技能。核心流程为三段式：
