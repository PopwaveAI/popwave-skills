...
**问题**：Step 3 门禁写死"主线超过1条→默认退回合并"，与 Step 0 用户确认的多主线结构直接矛盾。东方版深渊主宰项目中，用户在 Step 0 确认了3条主线，但 Step 3 门禁强制合并成1条，导致全链结构错配。

**改动**：
- `steps/step-3-plotline-docs.md`：删除"主线超过1条→退回合并"门禁，新增「三主线默认结构」章节（主线1世界危机/主线2主角成长/主线3主角行动），新增成长线和行动线的定义门禁（练物变强才是成长，杀人夺资源才是行动）
- `SKILL.md` Step 0：默认结构从"例如"改为明确推荐三主线+职责定义；门禁改为"你定吧→按三主线执行但需告知确认"
- `SKILL.md` Step 4：门禁从"每幕至少1主线+2副线"改为"每幕至少1主线活跃释放+2副线活跃"
- `steps/step-1-volume-strategy.md`：线数决策表门禁从"主线1条"改为"主线数量（默认3条）"
- `steps/step-4-act-plan.md`：活跃线标注增加三主线结构说明
- `templates/volume-outline.md`：线数配置建议表从"1主线+多支线"改为"3主线+多支线"；线数决策表主线默认数量从1改为3

**效果**：三主线并行成为默认结构，不再被门禁强制合并。成长线和行动线有了可量化的验收标准。
