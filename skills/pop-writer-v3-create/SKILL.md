...
> 由 pop-writer-v3-emerge Step 2 调度，context隔离。
...
| 编号 | 红线 | 检查方式 |
|------|------|----------|
| 1 | context隔离——传入精简context，不传会话历史 | emerge调度时只传该步骤所需最小context |
...
- **输入：** 种子/_index.yaml+种子六要素文件（压力矩阵/主角引擎/金手指/冲突轴/成长路径/目的地）+活记忆+上章末尾+chapter_plan+info_acquired+创作模板
- **输出：** 正文初稿+创作决策记录

## 路由

upstream `pop-writer-v3-emerge`（调度器，Step 2组装精简context调用本skill） → **本skill** → downstream `pop-writer-v3-revise`（修订子skill，接收正文初稿）

**调度架构：** 本skill由emerge调度器Step 2调用，context隔离——只接收精简context，不接收会话历史。创作完成后返回正文初稿+创作决策记录给emerge调度器，由调度器传递给Step 3调度revise。
