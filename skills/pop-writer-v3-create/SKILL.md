...
- **输入：** 种子/_index.yaml+种子六要素文件（压力矩阵/主角引擎/金手指/冲突轴/成长路径/目的地）+活记忆+上章末尾+种子文档.md（统一活文档，含任务表[主线/压力/线索任务]+要素切片+本章聚焦）+info_acquired+创作模板
- **输出：** 正文初稿+创作决策记录（含本章聚焦调整记录，如有）

## 种子文档写入权限

create在涌现写作过程中，仅可更新种子文档的"本章聚焦"段。不得修改任务表其他段（主线/压力/线索任务）。

| 种子文档段 | create权限 | 维护者 |
|:-----------|:-----------|:-------|
| 主线任务 | 只读 | expert-writer Step0+arc |
| 压力任务 | 只读 | expert-writer Step0+Step4 |
| 线索任务 | 只读 | expert-writer Step0+Step4 |
...
v1.2.0 | 2026-06-27 | v3.5：输入适配为种子文档.md（统一活文档，含任务表+要素切片+本章聚焦）；新增种子文档本章聚焦写入权限
