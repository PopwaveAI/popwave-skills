...
description: 涌现写作修订子skill（v3.5渲染层）。context隔离，完全重写模式+文风DNA硬阻塞（Get-Content -Raw完整加载）+文风DNA终验（7项附证据）+事实信息一致性检查（6项零改写）+导演意图验证（6项，v1.3新增世界观传递验证）+AI观感词清理+字数终检。输入来源从种子文档改为写作参考/设定/（主角引擎+金手指）+L2卡活跃线索。context receipt预留接口（批次2完整实现）。
pipeline:
