# CHANGELOG — 06-pop-novel-world

## v1.2.1 — 2026-06-14

- v5 structural refactoring: added WRONG examples table (3 entries), templates/ directory with L1-01~06 tpl files, combat-capability.tpl.yaml, starting-snapshot.tpl.md, step files for Phase 1-9, steps references in SKILL.md, pipeline field in skill.json.

## v1.2.0 — 2026-06-12

### 数据采集层移回 creative
- Phase W0（跨域素材聚合）和 Phase W1（拆书融合）移回 05-pop-novel-creative 并前置
- world 简化为六阶段：Phase 1 L1设定 → 2 稳定性 → 3 角色卡 → 4 数值 → 5 起点快照 → 6 宪法
- creative 移交数据产物（跨域素材蒸馏 + 拆书融合摘要），world 直接消费
- 质量红线从 10 条减为 8 条（移除 W0/W1 相关红线）
- 相位文件索引从 8 项减为 6 项

### creative 联动
- creative v1.4.0: W0/W1 回流并前置到 Phase 0 元素融合之前

---

## v1.1.0 — 2026-06-12

### 新增 Phase W0/W1：数据采集层移入
- Phase W0（跨域素材聚合，HARD GATE）和 Phase W1（拆书融合，有对标书时强制）从 05-pop-novel-creative 移入
- 原 Phase 1-5 重编号为 W2-W7
- 创意层（creative）在样品签字后使命完成——数据采集归 world
- 不强制拆书：只有对标书时才强制执行 Phase W1

### 关键决策
- 跨域素材始终强制（不管有没有对标书）
- 拆书融合有对标书时强制执行（不看 = 假对标 → 退回 creative）
- Phase W0/W1 放在 L1 设定之前：原料齐了才开始盖楼

---

## v1.0.0 — 2026-06-12

### 初始发布
- 从 pop-novel-bookstrap 拆出，聚焦"从宪法推导世界参数"
- 消费 05-pop-novel-creative 的故事引擎.md
- 消费 04-pop-novel-character-schema 的 Lv1~Lv4 模板产出角色卡
- 消费 deconstructor T1/T2/T3/T4（如有参考书）

### 产出物
- L1 设定层六件套
- 世界稳定性检验报告
- 角色卡初版
- 数值体系四件套
- 起点快照
- 世界宪法文档

### 关键决策
- 数值体系归 world（不是工业决策——是世界规则）
- 终点快照归 plot
- reader_profile 归 creative
- character-schema 作为独立标准 skill 被 world 消费
