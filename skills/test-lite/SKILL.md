---
name: test-lite
description: "轻量写作实验·通用小说创作引擎。当用户说'lite写作''轻量写作''test-lite''写小说/写同人/写剧本/构筑世界观(轻量路线)'时启用。单文件自包含无steps：场景识别(§2)→五维角色卡+灵魂场(§3)→叙事逻辑+3:1 pacing(§4)→五档风格温度(§5)→世界构筑六步(§6)→技法原则(§7)→反AI 13纹(§8)→同人模块(§9)→长篇状态管理(§10)。实验性质：验证超轻量单文件skill效果。"
---

# Novel Writing

> test-lite（实验）：单文件超轻量写作 skill——无 steps/templates/references 资源分层，全部方法论自包含于本文件。用于验证「非常轻度的 skill」的实际效果。
...
v0.1.0 | 2026-08-20
- 初始版本：移植 story-craft 单文件文档为 test-lite，验证超轻量 skill（无 steps/templates/references 资源分层）的实际效果
- 仅本地化 frontmatter（name/description 改 test 系列触发风格），正文 §0–§12 完整保留原文
