...
Tokens entering context to answer **one** targeted question. tool-book-to-skill loads a
resident core (~4K) plus one compiled chapter (~1K) ≈ **5,000 tokens**.

| Book (chapter size) | Context-dump | Discovery loop | tool-book-to-skill | vs dump / loop |
|---------------------|-------------:|---------------:|--------------:|:--------------:|
