...
into an agent's context to answer one targeted question:
  ...
  3. tool-book-to-skill  — a small resident SKILL.md core + one pre-compiled chapter
                      loaded on demand.
...
Usage:
  python3 scripts/discovery_tax.py --full-text <full_text.txt> \
...
def main() -> int:
    ...
    # tool-book-to-skill resident cost
    if args.skill_dir:
    ...
    print(f"    tool-book-to-skill     : {skill:>9,}   core [{core_label}] ({core:,}) + compiled chapter ({comp_chapter:,})\n")

    print("  tool-book-to-skill advantage:")
