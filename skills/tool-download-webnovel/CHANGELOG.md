...
### SKILL.md 重构为三阶段 SOP + 搜索链路 bug 修复

**需求：** 用户反馈"已知源太少了，源不够 agent 直接放弃继续努力了"。根因不是源少（笔趣阁镜像站频繁生死），而是 SKILL.md 没有教 agent 在内置源失败后如何动态找源——agent 看到 `status=error` 就直接放弃了。

**核心改动 1：SKILL.md 重构为动态 SOP**
- **Phase 1: 脚本自动搜索** — 内置源自动搜索+验证+爬取
- **Phase 2: Agent Web 搜索（强制）** — Phase 1 失败后，agent 必须用 web 搜索找外部免费源，至少尝试 3 组关键词
- **Phase 3: 验证与交付** — 检查 JSON 输出，处理各种 warning
- 红线新增："禁止在 Phase 1 脚本搜索失败后直接放弃"

**核心改动 2：搜索链路 bug 修复（download_novel.py）**
1. 新增 xbiquge.la 为首要可搜索源（Jieqi CMS，重定向到 xbiqugu.la）
2. 修复 `is_author_index_page()` 误判：
   - "全部小说"/"全部作品"从标记列表移除（出现在正常书籍页的导航菜单中）
   - TXT下载≥5 不再单独判定为作品集页（80ge 下载站的书页面有 10+ 个"TXT下载"按钮）
   - 正向检查：≥10 个"第X章"链接 → 一定是章节列表，不是作品集页
   - 标记检查优先于正向检查（作品集页可能列出多本书的最新章节"第X章"）
3. 修复 `parse_search_results()` fallback 太松：
   - 主匹配改为单向（标题 in 结果文本），不再反向匹配（短文本 in 长标题导致误匹配）
   - fallback 从"返回所有 URL 匹配的链接"改为"至少 2 字符连续匹配"
4. 调整源优先级：xbiquge 排在 80ge 前面（80ge 是下载站，不适合章节爬取）
5. 修复内容校验误报：搜索阶段已做作者验证的，下载后不再重复校验作者名（章节正文不含作者名）
6. 书名校验降为软提示（"笔趣阁改名属正常现象，请检查preview确认"）

**实测结果：** 斗破苍穹 + `--author "天蚕土豆"` → xbiquge 搜索 24 个候选 → 候选 24 通过作者验证 → 1624 章链接 → 3 章测试全部成功 → 内容校验无误报

## v6.2.0 (2026-07-16)

### 笔趣阁站点兼容性优化

**需求：** 下载《我在美国搞内战》时，v6.1.0 验证了内置源确实没收录，但通过 web search 找到 biquge365.net 有该书（改名为"我在美国拼高达"防搜索）。下载后发现两个问题：①章节列表倒序（最新章在前）②每章混入大量导航文字（首页/章节报错/存书签/关灯/一秒记住等）。

**改动（download_novel.py）：**
1. 新增 `--reverse` 参数：反转章节顺序，解决笔趣阁站最新章排在前面的问题
2. 新增 `CONTENT_NOISE_PATTERNS` 列表 + 在 `_clean_chapter_text()` 中应用：自动清除 13 种常见笔趣阁导航/广告文字（首页/返回/关灯/护眼/字号/章节报错/上一章/目录/存书签/下一章/一秒记住/biquge域名广告/分页指示器）
3. `div.txt` 加入 `FALLBACK_CONTENT_SELECTORS` 列表（biquge365 正文选择器）
4. 新增 biquge365 为已知源（含域名映射、选择器配置）
5. `NAV_LABELS` 扩展：添加"返回/关灯/护眼/字号/章节报错/存书签/免登陆/一秒记住"

**SKILL.md 同步更新：**
- 参数表新增 `--reverse`
- 已知源表新增 biquge365
- 内容校验告警逻辑优化：笔趣阁站改名不误判，agent 应先检查预览正文
- 异常处理表更新内容校验告警处理方式
