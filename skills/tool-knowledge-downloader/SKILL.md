...
| # | 错误 | 正确 |
|:-:|:-----|:-----|
| W1 | 微信文章下载后不检查正文完整性，直接进入 Phase B 拆解 | 下载后先校验正文是否完整（有无截断/乱码），确认无误再拆解 |
| W2 | B站字幕获取后不做清洗（保留时间戳/语气词/重复行），直接做拆解 | 字幕必须清洗：去时间戳、去语气词、合并断句，再进入拆解 |
| W3 | Phase A 下载未完成就进入 Phase B 生成拆解报告 | Phase A 必须全部完成（下载 + 校验）后，才允许进入 Phase B |
