...
所有产出的 PRD.md 应遵循 `references/prd-document-spec.md` 定义的统一规范：
- **元数据块**：版本/日期/状态/分类/关联
- **段落模板**：一句话范围 → 问题诊断 → 方案 → 不做的事 → 验收条件
- **证据链**：有数据、有对比、可复现（问题诊断类必选）
- **修复标记**：`✅ 已修复` / `⏳ 待执行` / `❌ 不做` / `⛔ 框架层问题`

详细规范 → `references/prd-document-spec.md`。
