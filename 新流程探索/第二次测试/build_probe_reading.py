...
METRICS = """| 条件 | 汉字 | 段数 | 平均段长 | 句长均值 | 短句(<10字)占比 | 破折号 | 省略号 |
|:--|--:|--:|--:|--:|--:|--:|--:|
...
流派特征词交叉命中（每万字，括号内为原始次数）：

| 条件 | 武侠词表 | 克苏鲁词表 | 规则怪谈词表 |
|:--|--:|--:|--:|
| C0 无线索词 | 0 | 0 | 0 |
| C1 武侠 | **35.3** (5) 剑法／江湖 | 0 | 0 |
| C2 克苏鲁 | 0 | **12.5** (2) 理智／腐败 | 0 |
| C3 规则怪谈 | 0 | 0 | **12.7** (3) 规则 |
| C4 生造词 | 0 | 0 | 0 |"""
...
{sections}
                                ...
                                form=md_table(METRICS.split("\n\n")[0]),
                                lex=md_table(METRICS.split("\n\n")[1]),
                                sections="\n".join(sections))
