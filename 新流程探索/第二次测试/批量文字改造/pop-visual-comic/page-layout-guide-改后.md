...
> Step 2 生成 HTML 的参考。画面由 AI 画（Seedream 直出完整漫画页），文字由 HTML 叠加（旁白条与对白气泡）。两者各司其职。
...
**v4.3.2**：从翻页阅读器改回长条滚动——纯 HTML 与 CSS，零 JS 交互，运行稳定且无缺陷。
...
| `seal.position` | string | 否 | 印章位置（`bottom-right` 或 `top-right`），默认 `bottom-right` |
| `pages_dir` | string | 是 | 页面图片目录（相对路径，通常为 `output`） |
| `output_html` | string | 是 | 输出 HTML 文件名（通常为 `index.html`） |
| `footer` | string | 否 | 页脚信息（默认 `未完待续 · popwave.cn 让创意一键落地`），钩子与品牌合并为单行 |
...
| `captions[].type` | string | 是 | `narration`（旁白条）或 `dialogue`（对白气泡） |
...
| `side-top` | narration | 顶部侧边窄条（左上，避开人脸与对白） |
| `side-bottom` | narration | 底部侧边窄条（左下，避开人脸与对白） |
...
> **铁律（v7.20.0）**：旁白条类名**只用 `top`、`bottom`、`side-top`、`side-bottom`**，CSS 须为这四个类定义定位规则。**不得使用 `top-left` 与 `top-right` 作为旁白条类名**（这两个类只给对白气泡用）——旁白条若用无定位规则的类名，会因缺少 `top` 锚点而错位落到页面底部之外（第4章 P1/P6 实测教训）。
...
**视觉规范**：
- 最大宽度 45%（不遮挡画面主体）
- **接近实色白底（rgba(255,255,255,0.97)）**——浅色或高亮画面上气泡仍清晰成型，不与背景相融
- **深色文字与白色描边**（color #1a1a1a + text-shadow 白边）——深字带白描边，在亮画面上与画面分隔，醒目
- 圆角 12px
- 字号 14px，行高 1.6
...
> 信息页由一半漫画图与一半文字块组成，**图文同页不可分离**（不得出纯文字无图的信息卡）。上漫画、下文字，文字承载浓密的世界观与背景，画面提供视觉锚点。信息页设计与穿插节奏由导演卡 `step1-director-card.md` §3.1c 规划。
...
> **铁律**：信息页须有图（`info-art` 不可为空）。信息页前后须各有漫画页，不得连续 2 个信息页。信息页占全章页数 ≤30%。
...
| 规则 | 说明 |
|:-----|:-----|
| 避让人物面部 | 对白与旁白不得覆盖角色脸部区域 |
| 旁白条只占边缘 | 顶部与底部旁白条高度 ≤6%，只放 `top`、`bottom`、`side-top`、`side-bottom` 四类，不覆盖画面中心动作 |
| 对白气泡放角落 | 只放四角，max-width 45%，不压脸 |
| 不得使用无规则类名 | 旁白条不得使用 `top-left` 与 `top-right`（那是对白类），否则错位落底部之外 |
| 大单页慎用文字 | 名场面画面自说，除非导演卡明确标注旁白 |
| 旁白条 ≤3 行 | 超过 3 行用导演卡拆分为多页或精简旁白 |
| 对白气泡 ≤15 字 | 超长对白拆分为多个气泡或精简 |

> **文字安全区（v7.20.0）**：导演卡设计页面时，为每页预留**顶部 6% 与底部 6% 的文字安全带**。角色脸部与关键动作放在画面中部，旁白条只放在安全带内——从源头避免"文字压脸"。若某页角色脸须贴近上边缘或下边缘，则该页旁白改用 `side-top`、`side-bottom` 窄条，或干脆不放该位置。
...
> **直接复制模板，替换 `{章节标题}`、`{章节名}`、`pageN.png`、`{旁白文字}`、`{对白文字}`、`{slogan}`、`{footer}` 等占位符即可。** 纯 HTML 与 CSS，无 JavaScript，向下滚动浏览。

## 品牌水印（v7.6.0）

> 2026-08-04 定稿：品牌署名走 HTML 工程化方案（历史已有教训：提示词融水印不稳定，见 CHANGELOG）。品牌水印**不逐页压**，避免破坏画面冲击力。
...
**结尾页脚单行**（未完待续与 slogan 合并，不占两行——老板反馈页脚两行过挤）：
...
- 不使用 `powered by`（太像开源软件署名与技术声明）
- slogan 点出"让创意一键落地"的 AI 自动代劳卖点，品牌 `popwave.cn` 前置
- 结尾 `未完待续` 是追读钩子，与 slogan 合并单行，避免两行过挤

### 设计原则

- **不逐页压角标**：长条滚动漫画是沉浸阅读，每页压水印会破坏画面冲击力（历史问题：提示词融水印 → 不稳定，改走 HTML）
