...
1. **画风前置**：画风DNA放第2段（高权重位），不放开头也不放末尾。Seedream 注意力权重分配规律表明，前置信息执行力更强
...
5. 指定单一主光源
6. 风格词放末尾（注意：V3 中风格在末尾，但 6 段式中画风在第 2 段，两者适用场景不同）
...
- **块1 锁定构图**：镜头规格用具体焦段（"24mm广角"比"广角"精确）；角色外观写皮肤质感和毛孔级细节；服装从外到内逐层写；每只手写具体动作；背景按远景、中景、近景标注虚实
...
3. 取推荐光照和构图：`recommended_lighting` 与 `recommended_composition`
4. 兼容性检查：查 `lighting-composition-templates.md` 兼容性矩阵

### 跨 skill 引用协议

cover/oc/comic 从 DNA 库取画风时：
- `dna` 与 `constraint` 用于提示词的画风段
- `recommended_lighting` 用作光照兼容性参考（不替代各 skill 自己的光源设计）
- `recommended_composition` 用作构图参考（不替代各 skill 自己的构图体系）
...
1. **用自然语言清晰描述画面**：简洁连贯地写明主体、行为、环境与风格
...
6. **图生图编辑用明确指令**：说明修改或参考的对象及具体操作，避免模糊代词
...
当用户参考点为"画风"时，默认使用放开吸收公式。核心是**正向吸收与最小排除**：不要堆"不参考"清单，否则等于没参考（实测吸收被抑制）。
...
**关键原则**：
1. 正向吸收：明确列出要吸收的要素，包括画风质感（笔触、渲染）、色彩系统（色调倾向、饱和度、明暗）、光影氛围（光源、氛围、空气感）、人物精致度
2. 最小排除：只排除两样，即具体场景内容（场景物品、环境元素）和人物长相（五官、脸型）。姿态、服饰、构图、色彩都允许参考图自然传导
