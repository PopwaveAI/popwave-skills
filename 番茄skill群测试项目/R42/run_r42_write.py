...
"""
R42 · 试写脚本
策略：每本书 = 文风DNA + ch03中性剧情梗概 → API → 正文
两本书并行试写，对比笔触一致性。
同时落盘完整input（system+user prompt）。
"""
...
INPUT_LOG_DIR = os.path.join(OUTPUT_DIR, "inputs")  # 完整input落盘
os.makedirs(INPUT_LOG_DIR, exist_ok=True)

BOOKS = ["玄鉴仙族", "夜无疆"]

# ========== write skill 核心约束（从SKILL.md/step1.md提取）==========
...
# 格式约束
- 章名：正常章节名，贴合内容命名，不刻意制造噱头
- 系统面板用【】内联格式，禁止markdown代码块
- 角色思考用碎片化表达，避免完整分析句
- 字数：2000-2500字，不准超过3000字
...
请根据上述剧情梗概，用文风DNA驱动的笔触，写出第3章完整正文。

要求：
1. 笔触必须从DNA中来——叙事距离、句式节奏、感官顺序、对话引导词、情绪外化方式，都按DNA描述的特征来写
2. 字数2000-2500字，不准超过3000字
3. 章名用正常章节名（贴合内容）
...
直接输出正文（含章名），不需要解释。"""
    ...
    # 落盘完整input（system+user）
    input_log_path = os.path.join(INPUT_LOG_DIR, f"{book_name}-ch03_input.md")
    ...
    try:
        ...
        # 落盘正文
        out_path = os.path.join(OUTPUT_DIR, f"{book_name}-ch03试写.md")
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        futures = {executor.submit(write_chapter, name): name for name in BOOKS}
    ...
    with open(os.path.join(OUTPUT_DIR, "meta.json"), "w", encoding="utf-8") as f:
        json.dump({"round": "R42", "task": "write_test", "books": metas}, f, ensure_ascii=False, indent=2)
