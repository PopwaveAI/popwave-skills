...
# -*- coding: utf-8 -*-
"""
R42c · write skill完整约束对比测试
策略：
  - 不注入文风DNA（trial模式）
  - 注入write skill完整约束（红线+章型+节奏+爽感+微观技法+五层指导+写作禁区）
  - 输入两本各自的剧情梗概
  - 对比R42原版（DNA+简化约束）看质量差异
两本书并行。
"""
...
WS_INPUT_LOG_DIR = os.path.join(OUTPUT_DIR, "inputs-ws")  # write skill版input落盘
os.makedirs(WS_INPUT_LOG_DIR, exist_ok=True)

BOOKS = ["玄鉴仙族", "夜无疆"]

# ========== write skill 完整约束（trial模式·无DNA）==========
SYSTEM_PROMPT = """你是番茄小说正文写作agent，严格遵循pop-fanqie-write skill v7.4.0 的完整约束。

# 当前模式：trial模式（无笔触DNA）
未检测到文风DNA（涌现/文风锚定.md不存在）。仅允许使用通用笔触约束，不得伪装成DNA驱动。
...
11. 笔触DNA标准插槽：当前为trial模式，仅用通用笔触约束
...
def write_ws(book_name):
    """write skill版试写（trial模式·无DNA）"""
    print(f"\n[{book_name}] 开始write skill版试写...")
...
请根据上述剧情梗概，按pop-fanqie-write skill完整约束，写出第3章完整正文。

要求：
1. 当前为trial模式（无笔触DNA），用通用笔触约束
2. 从4种章型中选1个最贴合本章剧情的，按7节拍骨架组织结构
3. 选3-4类微观技法（含动作链+多感官优先级必选）并落地
4. 字数2000-2500字，不准超过2500字
5. 章名用正常章节名（贴合内容）
6. 章末钩子（主角主动决策型）
7. 系统面板用【】内联格式
8. 角色思考碎片化表达
...
直接输出正文（含章名），不需要解释，不需要写思考过程。"""
    ...
    # 落盘完整input
    input_log_path = os.path.join(WS_INPUT_LOG_DIR, f"{book_name}-ws_input.md")
    ...
    try:
        ...
        out_path = os.path.join(OUTPUT_DIR, f"ws-{book_name}-ch03.md")
...
def main():
    print("="*60)
    print("R42c · write skill完整约束对比测试（trial模式·无DNA）")
    print("="*60)
    
    metas = []
    with ThreadPoolExecutor(max_workers=2) as executor:
        futures = {executor.submit(write_ws, name): name for name in BOOKS}
    ...
    with open(os.path.join(OUTPUT_DIR, "meta-ws.json"), "w", encoding="utf-8") as f:
        json.dump({"round": "R42c", "task": "write_skill_test", "mode": "trial-no-dna", "books": metas}, f, ensure_ascii=False, indent=2)
    
    print("\n" + "="*60)
    print("write skill版试写完成!")
    for m in metas:
        if "error" in m:
