...
R42f · 公平测试（排除原文片段干扰）
策略：
  轮1（同源·公平）：用ch50梗概（不在DNA采样范围内）+ 各自DNA
    - 玄鉴ch50 + 玄鉴DNA
    - 夜无疆ch50 + 夜无疆DNA
  轮2（交叉·公平）：用深渊主宰ch075梗概（与两本DNA完全不重叠）+ 两本DNA
    - 深渊主宰ch075 + 玄鉴DNA
    - 深渊主宰ch075 + 夜无疆DNA
  4个任务并行。
按三标准测评：1.遵照剧情白描 2.笔触对标原文 3.读下来精彩
"""
...
R42F_INPUT_LOG_DIR = os.path.join(OUTPUT_DIR, "inputs-r42f")
os.makedirs(R42F_INPUT_LOG_DIR, exist_ok=True)

# 4个任务：(任务名, 剧情来源, DNA来源)
TASKS = [
    ("f1-玄鉴ch50+玄鉴DNA", "玄鉴仙族-ch50", "玄鉴仙族"),
    ("f2-夜无疆ch50+夜无疆DNA", "夜无疆-ch50", "夜无疆"),
    ("f3-深渊ch075+玄鉴DNA", "深渊主宰-ch075", "玄鉴仙族"),
    ("f4-深渊ch075+夜无疆DNA", "深渊主宰-ch075", "夜无疆"),
]

# ========== write skill v8.0.0 结构约束 + DNA启用态 ==========
SYSTEM_PROMPT = """你是番茄小说正文写作agent，严格遵循pop-fanqie-write skill v8.0.0 的结构约束，并加载文风DNA驱动笔触。
...
4. 篇幅硬限制：2000-2500字，不准超过2500字
5. 章型必选：每章必须从4种章型中选1个，按7节拍骨架组织结构
6. 爽感≥1/章：每章至少1个爽感触发
7. 章名：每章正文开头必须有正常章节名（贴合内容命名）
8. 笔触DNA标准插槽：当前为启用态，笔触层必须从DNA中来
...
def write_task(task_name, synopsis_source, dna_source):
    """R42f单任务"""
    print(f"\n[{task_name}] 开始...")
    
    # 加载DNA
    dna_path = os.path.join(DNA_DIR, f"{dna_source}-文风DNA.md")
    with open(dna_path, "r", encoding="utf-8") as f:
        dna = f.read()
    print(f"[{task_name}] DNA来源: {dna_source} ({len(dna)}字符)")
    
    # 加载剧情梗概
    synopsis_path = os.path.join(INPUT_DIR, f"{synopsis_source}剧情梗概.md")
    with open(synopsis_path, "r", encoding="utf-8") as f:
        synopsis = f.read()
    print(f"[{task_name}] 剧情来源: {synopsis_source} ({len(synopsis)}字符)")
...
请根据上述剧情梗概，按pop-fanqie-write skill v8.0.0结构约束 + 文风DNA驱动笔触，写出完整正文。
...
直接输出正文（含章名），不需要解释。"""
    ...
    # 落盘完整input
    input_log_path = os.path.join(R42F_INPUT_LOG_DIR, f"{task_name}_input.md")
    ...
    try:
        ...
        out_path = os.path.join(OUTPUT_DIR, f"r42f-{task_name}.md")
...
def main():
    print("="*60)
    print("R42f · 公平测试（排除原文片段干扰）")
    print("="*60)
    
    metas = []
    with ThreadPoolExecutor(max_workers=4) as executor:
        futures = {executor.submit(write_task, name, syn, dna): name 
                   for name, syn, dna in TASKS}
    ...
    with open(os.path.join(OUTPUT_DIR, "meta-r42f.json"), "w", encoding="utf-8") as f:
        json.dump({"round": "R42f", "task": "fair_test", "tasks": metas}, f, ensure_ascii=False, indent=2)
