...
# -*- coding: utf-8 -*-
"""
R40 seed测试：3段API调用验证seed全流程

段1: 发散——给定赛道，产出10个创意（双轨），自动选1个
段2: 打磨——选定创意→产出创意.md（金手指+行为引擎+故事纲领+番茄简介）
段3: 首章——创意.md→产出ch001.md

验收目标：
1. 创意精彩度（段1发散质量）
2. 第一章精彩度（段3首章质量）
3. 立项PRD对world承接性（段2创意.md完整性）
"""
...
def main():
    ...
    print("=" * 60)
    print(f"R40 seed测试：3段API调用验证seed全流程")
    print(f"赛道：{TRACK}")
    print("=" * 60)
    ...
    try:
        ...
        p1_path = save_input_output("phase1-发散", PHASE1_SYSTEM, PHASE1_PROMPT, p1_output, p1_usage, p1_elapsed)
        meta["phases"].append({
            "name": "phase1-发散",
    ...
    try:
        ...
        p2_path = save_input_output("phase2-创意", PHASE2_SYSTEM, p2_prompt, p2_output, p2_usage, p2_elapsed)
        meta["phases"].append({
            "name": "phase2-创意",
    ...
    try:
        ...
        p3_path = save_input_output("phase3-ch001", PHASE3_SYSTEM, p3_prompt, p3_output, p3_usage, p3_elapsed)
        meta["phases"].append({
            "name": "phase3-ch001",
    ...
    # 保存meta
    meta_path = os.path.join(OUTPUT_DIR, "r40-meta.json")
    with open(meta_path, "w", encoding="utf-8") as f:
        json.dump(meta, f, ensure_ascii=False, indent=2)

    print(f"\n输出目录: {OUTPUT_DIR}")
    print(f"\n下一步: 人工复盘验收三个目标")
    print(f"  1. 创意精彩度 → output/phase1-发散.md")
    print(f"  2. 立项PRD对world承接性 → output/phase2-创意.md")
    print(f"  3. 第一章精彩度 → output/phase3-ch001.md")
