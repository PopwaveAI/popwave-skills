...
# -*- coding: utf-8 -*-
"""
R23测试: 番茄write v7.0.0验证
- 复用R22的seed和plot产出（控制变量）
- 注入笔触DNA（测试加载机制）
- 只跑write ch01，对比R22的ch01产出
- 验证：①字数控制 ②章型骨架 ③微观技法 ④笔触DNA绑定
"""
...
def main():
    ...
    print("=" * 60)
    print("R23测试: 番茄write v7.0.0验证")
    print("复用R22的seed+plot产出 + 注入笔触DNA")
    print("=" * 60)

    # ========== 加载R22产出 ==========
    print(f"\n[0] 加载R22产出...")
    r22_seed_path = os.path.join(R22_OUTPUT, "r22-seed.md")
    r22_plot_path = os.path.join(R22_OUTPUT, "r22-plot.md")
    dna_path = os.path.join(INPUT_DIR, "笔触DNA.md")

    with open(r22_seed_path, "r", encoding="utf-8") as f:
        seed_content = f.read()
    with open(r22_plot_path, "r", encoding="utf-8") as f:
        plot_content = f.read()
    with open(dna_path, "r", encoding="utf-8") as f:
        dna_content = f.read()

    print(f"  Seed: {len(seed_content)}字")
    print(f"  Plot: {len(plot_content)}字")
    print(f"  笔触DNA: {len(dna_content)}字")

    # ========== Write ch01 (v7.0.0 + 笔触DNA) ==========
    print(f"\n[1/1] 调用DeepSeek执行Write ch01 (v7.0.0+笔触DNA)...")
...
5步SOP：
- Step 1: 加载燃料（创意+骨架+笔触DNA+心法分支+黄金开篇）
...
"""
    ...
    try:
        ...
        write_path = os.path.join(OUTPUT_DIR, "r23-ch01.md")
    ...
    # ========== 汇总 ==========
    meta_path = os.path.join(OUTPUT_DIR, "r23-meta.json")
