...
# -*- coding: utf-8 -*-
"""
R32 AB测试: write v7.2.0融合方案 vs 外部PE
用深渊主宰C方案作为统一输入，对比两者正文质量

A组: write v7.2.0 skill（5步SOP+章型+爽感引擎+精选设定库+动作链+多感官+章名设计）
B组: 外部PE（4段式骨架+19条规则+章节名要求）

与R31的区别：A组升级到v7.2.0（新增动作链+多感官+章名）
"""
    ...
    for ch_idx in range(1, MAX_CHAPTERS + 1):
        ...
        try:
            ...
            content, usage, elapsed = call_ds(
                system_prompt, user_prompt, max_tokens=max_tokens, label=f"R32_{group_name}_{ch_num}"
            )
...
def main():
    ...
    print("=" * 60)
    print("R32 AB测试: write v7.2.0融合方案 vs 外部PE")
    ...
    # A组
    a_output = os.path.join(OUTPUT_DIR, "A-write-v720")
    os.makedirs(a_output, exist_ok=True)
    a_results = run_group("A", WRITE_SYSTEM_V72, build_prompt_a, decon_files, a_output)
    ...
    meta = {
        "config": {
            ...
            "input": "深渊主宰C方案（拆书产出）",
            "A_version": "write v7.2.0（融合方案：动作链+多感官+章名）",
            "B_version": "外部PE（与R31一致）"
        },
        "A_write_v720": a_results,
        "B_external_pe": b_results
    }
    meta_path = os.path.join(OUTPUT_DIR, "r32-meta.json")
